...
//------------------------------------------------------------------------------
//
// Purpose: Heretic intermission screens.
...
static void HI_loadData(void)
{
...

    {
        ...
        else
        {
            ...
            {
                psnprintf(nameBuffer, 24, "_IN_NAME_E%01dM%01d", hi_wbs.nextEpisode + 1, hi_wbs.next + 1);
                if((str = E_StringForName(nameBuffer)))
                    nextMapName = str->string;
            }
    ...
    if(hi_wbs.nextEpisode == hi_wbs.epsd)
    {
        for(i = 0; i <= previous; i++)
        {
            if(i >= NUM_LEVELS_PER_EPISODE)
                break;
            V_DrawPatch(hipoints[hi_wbs.epsd][i].x, hipoints[hi_wbs.epsd][i].y, &subscreen43, hi_in_x);
        }

        // draw patch on secret level
        if(hi_wbs.didsecret)
        {
            V_DrawPatch(hipoints[hi_wbs.epsd][8].x, hipoints[hi_wbs.epsd][8].y, &subscreen43, hi_in_x);
        }
    }
    // blink destination arrow
    if(flashtime && hi_wbs.next >= 0 && hi_wbs.next < NUM_LEVELS_PER_EPISODE)
    {
        V_DrawPatch(hipoints[hi_wbs.nextEpisode][hi_wbs.next].x, hipoints[hi_wbs.nextEpisode][hi_wbs.next].y,
                    &subscreen43, hi_in_yah);
    }
}
...
static void HI_drawLeaving(void)
{
    ...
    {
        V_DrawPatch(hipoints[hi_wbs.epsd][thislevel].x, hipoints[hi_wbs.epsd][thislevel].y, &subscreen43, hi_in_x);
    }
}
...
static void HI_drawSingleStats(void)
{
    ...
    if(intertime > 30)
    {
        ...
        HI_drawLevelStat(players[consoleplayer].killcount, hi_wbs.maxkills, 200, 65);
    }
    ...
    if(intertime > 60)
    {
        ...
        HI_drawLevelStat(players[consoleplayer].itemcount, hi_wbs.maxitems, 200, 90);
    }
    ...
    if(intertime > 90)
    {
        ...
        HI_drawLevelStat(players[consoleplayer].secretcount, hi_wbs.maxsecret, 200, 115);
    }
...
static void HI_drawCoopStats(void)
{
    ...
    for(i = 0; i < MAXPLAYERS; ++i)
    {
        if(HI_playerInGame(i))
        {
            V_DrawPatchShadowed(25, ypos, &subscreen43, PatchLoader::CacheNum(wGlobalDir, hi_faces[i], PU_CACHE),
                                nullptr, FRACUNIT);
...
static void HI_drawDMStats(void)
{
    ...
    if(intertime < 20)
    {
        ...
        for(i = 0; i < MAXPLAYERS; ++i)
        {
            if(HI_playerInGame(i))
            {
                V_DrawPatchGeneral(40, (ypos * FRACUNIT + dSlideY[i] * intertime) >> FRACBITS, &subscreen43,
                                   PatchLoader::CacheNum(wGlobalDir, hi_faces[i], PU_CACHE), false);
                V_DrawPatchGeneral((xpos * FRACUNIT + dSlideX[i] * intertime) >> FRACBITS, 18, &subscreen43,
                                   PatchLoader::CacheNum(wGlobalDir, hi_dead_faces[i], PU_CACHE), false);
            }
        }
    }
    ...
    if(intertime >= 20)
    {
        ...
        for(i = 0; i < MAXPLAYERS; ++i)
        {
            if(HI_playerInGame(i))
            {
                ...
                if(i == consoleplayer)
                {
                    V_ColorBlockTLScaled(&subscreen43, blockcolor, xpos, ypos + 3, (263 - (xpos - 3) + 1) + 48, 36 - 5,
                                         FRACUNIT / 3);
                }

                V_DrawPatchTL(40, ypos, &subscreen43, PatchLoader::CacheNum(wGlobalDir, hi_faces[i], PU_CACHE), nullptr,
                              tllevel);
                V_DrawPatchTL(xpos, 18, &subscreen43, PatchLoader::CacheNum(wGlobalDir, hi_dead_faces[i], PU_CACHE),
                              nullptr, tllevel);
...
static void HI_Drawer()
{
    ...
    if(interstate == INTR_WAITING)
    {
        IN_slamBackground(); // needed because D_Display puts pillarboxes
        return;
    }
