...
public:
    ...
    explicit qstring(size_t startSize = 0) : ZoneObject(), index(0), size(16)
    {
        buffer = local;
    ...
    qstring(const qstring &other) : ZoneObject(), index(0), size(16)
    {
        buffer = local;
        memset(local, 0, sizeof(local));
        copy(other);
    ...
    explicit qstring(const char *cstr) : ZoneObject(), index(0), size(16)
    {
        buffer = local;
        memset(local, 0, sizeof(local));
        copy(cstr);
    ...
    friend qstring operator+(const char *a, const qstring &b) { return qstring(a).concat(b); }

    friend qstring operator/(const char *a, const qstring &b) { return qstring(a).pathConcatenate(b); }
