...
static Collection<PODCollection<sector_t *>> gSectorNeighborsThroughPortals;
...
void P_initSectorPortalNeighbors()
{
   ...
   for(int i = 0; i < numsectors; ++i)
   {
      ...
      // debug
      for(const sector_t *osector : coll)
      {
         printf("Sector %d has neigh %d\n", eindex(&sector - sectors), eindex(osector - sectors));
      }
   }
}
   ...
   for(int y = 0; y < bmapheight; y++)
   {
      for(int x = 0; x < bmapwidth; x++)
      {
         ...
         else for(; *tmplist != -1; tmplist++)
         {
            ...
            if(li.backsector && li.backsector->srf.ceiling.portal && li.backsector->srf.ceiling.portal->type == R_LINKED)
            {
               portalmap[writeOfs] |= PMF_CEILING;
               gMapHasSectorPortals = true;
            }
            ...
            if(li.backsector && li.backsector->srf.floor.portal && li.backsector->srf.floor.portal->type == R_LINKED)
            {
               portalmap[writeOfs] |= PMF_FLOOR;
               gMapHasSectorPortals = true;
            }
...
{
   gPolyGroupPairs = ecalloctag(decltype(gPolyGroupPairs), groupcount * groupcount,
                                sizeof(*gPolyGroupPairs), PU_LEVEL, nullptr);
   for(int i = 0; i < numPolyObjects; ++i)
   {
      const polyobj_t &poly = PolyObjects[i];
