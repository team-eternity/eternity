...
void HU_InventoryDrawSelector()
{
    ...
    while(E_MoveInventoryCursor(plyr, 1, i) && i < count)
    {
        ...
        ST_DrawSmallHereticNumber(E_GetItemOwnedAmount(plyr, artifact), origin + 19 + i * 31 + 8, ybase + 22, true);
    }
    ...
    if(leftoffs)
    {
        V_DrawPatch(origin - 12, ybase - 1, &vbscreenyscaled, !(leveltime & 4) ? leftarrows[0] : leftarrows[1]);
    }
...
void HU_BuffsDraw(int leftoffset, int rightoffset)
{
    ...
    static const char *const spinflylumps[] = {
        "SPFLY0", "SPFLY1", "SPFLY2",  "SPFLY3",  "SPFLY4",  "SPFLY5",  "SPFLY6",  "SPFLY7",
        "SPFLY8", "SPFLY9", "SPFLY10", "SPFLY11", "SPFLY12", "SPFLY13", "SPFLY14", "SPFLY15",
    };
    ...
    {
        if(plyr.powers[pw_weaponlevel2].infinite || plyr.powers[pw_weaponlevel2].tics > (4 * 32) ||
           !(plyr.powers[pw_weaponlevel2].tics & 16))
        {
