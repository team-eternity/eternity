...
static void R_renderSegLoop(cmapcontext_t &cmapcontext, planecontext_t &planecontext,
                            ...
                            drawseg_t *const ds_p, cb_seg_t &segclip, const cb_seg_t &fullseg)
{
#ifdef RANGECHECK
         ...
         if(line >= cliptop)
         {
            ...
            if(segclip.markflags & SEG_MARKCPORTAL)
            {
               R_WindowAdd(
                  ...
                  segclip.secwindow.ceiling, i, fullseg, (float)cliptop, (float)line
               );
      ...
      if(segclip.markflags & (SEG_MARKFPORTAL|SEG_MARKFLOOR|SEG_MARKFOVERLAY))
      {
         ...
         if(line <= clipbot)
         {
            ...
            if(segclip.markflags & SEG_MARKFPORTAL)
            {
               R_WindowAdd(
                  planecontext, portalcontext, heap, viewpoint, bounds,
                  segclip.secwindow.floor, i, fullseg, (float)line, (float)clipbot
               );
               floorclip[i] = (float)b;
            }
         ...
         if(!segclip.twosided)
         {
            if(segclip.l_window)
            {
               ...
               if(segclip.markflags & SEG_MARK1SLPORTAL)
               {
                  ...
                  R_WindowAdd(
                     planecontext, portalcontext, heap, viewpoint, bounds,
                     segclip.l_window, i, fullseg, ceilingclip[i], floorclip[i]
                  );
               }
               else
               {
                  R_WindowAdd(
                     planecontext, portalcontext, heap, viewpoint, bounds,
                     segclip.l_window, i, fullseg, (float)t, (float)b
                  );
               }
                      ...
                      const cb_seg_t &seg, const int start, const int stop)
{
   ...
   if(usesegloop)
   {
      ...
      R_renderSegLoop(
         cmapcontext, planecontext, portalcontext, heap, colfunc, viewpoint, cb_viewpoint, bounds, ds_p, segclip, seg
      );
   }
   else
