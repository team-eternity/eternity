                            ...
                            const int heightsec)
{
    ...
    wlight = ds->colormap[lightnum >= LIGHTLEVELS || cmapcontext.fixedcolormap ? LIGHTLEVELS - 1 :
                          lightnum < 0                                         ? 0 :
                                                                                 lightnum];
...
static const lighttable_t *R_calculateLighting(cmapcontext_t &cmapcontext, const lighttable_t *const *walllights,
                                               const float dist)
{
    // calculate lighting
    // SoM: ANYRES
...
//
static void R_renderSegLoop(cmapcontext_t &cmapcontext, planecontext_t &planecontext, portalcontext_t &portalcontext,
                            ZoneHeap &heap, const R_ColumnFunc colfunc, const viewpoint_t &viewpoint,
                            const cbviewpoint_t &cb_viewpoint, const contextbounds_t &bounds, drawseg_t *const ds_p,
                            cb_seg_t &segclip)
{
#ifdef RANGECHECK
    ...
    if(segclip.skyflat)
    {
        ...
        // to avoid HOM
        skyplane = R_FindPlane(cmapcontext, planecontext, heap, viewpoint, cb_viewpoint, bounds, viewpoint.z - 1,
                               segclip.skyflat, 144, {}, { 1, 1 }, 0, nullptr, 0, 255, nullptr);
    ...
    for(i = segclip.x1; i <= segclip.x2; i++)
    {
        ...
        if(segclip.markflags & (SEG_MARKCPORTAL | SEG_MARKCEILING | SEG_MARKCOVERLAY))
        {
            ...
            if(line >= cliptop)
            {
                ...
                {
                    R_WindowAdd(planecontext, portalcontext, heap, viewpoint, bounds, segclip.secwindow.ceiling, i,
        ...
        if(segclip.markflags & (SEG_MARKFPORTAL | SEG_MARKFLOOR | SEG_MARKFOVERLAY))
        {
            ...
            if(line <= clipbot)
            {
                ...
                {
                    R_WindowAdd(planecontext, portalcontext, heap, viewpoint, bounds, segclip.secwindow.floor, i,
                                (float)line, (float)clipbot);
                    floorclip[i] = (float)b;
                }
        ...
        if(segclip.segtextured)
        {
            ...
            if(ds_p->maskedtextureskew)
            {
                ...
                {
                    ds_p->maskedtextureskew[i] =
                        segclip.skew_mid_step * (segclip.len * basescale) * segclip.tscale_mid_y +
            ...
            if(!segclip.twosided)
            {
                if(segclip.l_window)
                {
                    ...
                    if(segclip.markflags & SEG_MARK1SLPORTAL)
                    {
                            ...
                            if(column.y2 >= column.y1)
                            {
                                ...
                                column.source =
                                    R_GetRawColumn(heap, segclip.toptex,
                                                   int((texx + segclip.toffset_seg_x) * segclip.tscale_top_x +
                            ...
                            if(column.y2 >= column.y1)
                            {
                                ...
                                column.source =
                                    R_GetRawColumn(heap, segclip.bottomtex,
                                                   int((texx + segclip.toffset_seg_x) * segclip.tscale_bottom_x +
                        ...
                        R_WindowAdd(planecontext, portalcontext, heap, viewpoint, bounds, segclip.l_window, i,
                                    ceilingclip[i], floorclip[i]);
                    }
                    else
                    {
                        R_WindowAdd(planecontext, portalcontext, heap, viewpoint, bounds, segclip.l_window, i, (float)t,
                                    (float)b);
                    }
                ...
                else if(segclip.midtex)
                {
                    ...
                    column.source    = R_GetRawColumn(heap, segclip.midtex,
                                                      int((texx + segclip.toffset_seg_x) * segclip.tscale_mid_x +
                                                       segclip.toffset_base_x + segclip.toffset_mid_x));
            ...
            else
            {
                if(segclip.t_window)
                {
                    ...
                    if(column.y2 >= column.y1)
                    {
                        R_WindowAdd(planecontext, portalcontext, heap, viewpoint, bounds, segclip.t_window, i,
                ...
                else if(segclip.toptex)
                {
                    ...
                    if(column.y2 >= column.y1)
                    {
                        ...
                        column.source    = R_GetRawColumn(heap, segclip.toptex,
                                                          int((texx + segclip.toffset_seg_x) * segclip.tscale_top_x +
                                                           segclip.toffset_base_x + segclip.toffset_top_x));
                        column.texheight = segclip.toptexh;
                        column.step      = M_FloatToFixed(basescale * segclip.tscale_top_y);
                ...
                if(segclip.b_window)
                {
                    ...
                    if(column.y2 >= column.y1)
                    {
                        R_WindowAdd(planecontext, portalcontext, heap, viewpoint, bounds, segclip.b_window, i,
                                    static_cast<float>(column.y1), static_cast<float>(column.y2));
                ...
                else if(segclip.bottomtex)
                {
                    ...
                    if(column.y2 >= column.y1)
                    {
                        ...
                        {
                            column.texmid += M_FloatToFixed(segclip.skew_bottom_step * (segclip.len * basescale) *
                                                                segclip.tscale_bottom_y +
        ...
        {
            R_WindowAdd(planecontext, portalcontext, heap, viewpoint, bounds, segclip.l_window, i, (float)t, (float)b);
            ceilingclip[i] = view.height - 1.0f;
            floorclip[i]   = 0.0f;
        }
        else if(skyplane)
...
static void R_storeTextureColumns(float *const maskedtexturecol, float *const maskedtextureskew, cb_seg_t &segclip)
{
    ...
    for(i = segclip.x1; i <= segclip.x2; i++)
    {
        ...
        if(maskedtextureskew)
        {
            if(segclip.skew_mid_step && segclip.side->middleSkewType() != SKEW_NONE)
            {
                maskedtextureskew[i] = segclip.skew_mid_step * (segclip.len * basescale) * segclip.tscale_mid_y +
                                       segclip.skew_mid_baseoffset;
            }
            else
                      ...
                      const int start, const int stop)
{
    ...
    usesegloop = !seg.backsec || seg.clipsolid || seg.markflags || seg.toptex || seg.midtex || seg.bottomtex ||
                 seg.f_portalignore || seg.c_portalignore || segclip.line->linedef->portal || ds_p->silhouette ||
                 !ds_p->maskedtexturecol;

    if(usesegloop)
    {
        const R_ColumnFunc colfunc = r_column_engine->DrawColumn;
        R_renderSegLoop(cmapcontext, planecontext, portalcontext, heap, colfunc, viewpoint, cb_viewpoint, bounds, ds_p,
                        segclip);
    }
    else
