...
//------------------------------------------------------------------------------
//
// Purpose: Archiving: SaveGame I/O.
...
{
    static_assert(sizeof(size_t) == SAVE_MIN_SIZEOF_SIZE_T || sizeof(size_t) == SAVE_MAX_SIZEOF_SIZE_T,
...
{
    *this << st.damage << st.damageflags << st.leakiness << st.damagemask << st.damagemod << st.flags << st.newspecial;
...
SaveArchive &SaveArchive::operator<<(mapthing_t &mt)
{
    ...
    *this << mt.angle << mt.height << mt.next << mt.options << mt.extOptions << mt.recordnum << mt.special << mt.tid
          << mt.type << mt.x << mt.y << mt.healthModifier;
...
{
    *this << zref.floor << zref.ceiling << zref.dropoff << zref.secfloor << zref.secceil << zref.passfloor
          << zref.passceil;
    if(saveVersion() >= 15)
    {
        int32_t val;
...
static void P_ArchivePlayers(SaveArchive &arc)
{
    for(int i = 0; i < MAXPLAYERS; i++)
    {
        ...
        if(playeringame[i])
        {
            ...
            arc << p.playerstate << p.cmd.actions << p.cmd.angleturn << p.cmd.chatchar << p.cmd.consistency
                << p.cmd.forwardmove << p.cmd.look << p.cmd.sidemove << p.viewz << p.viewheight << p.deltaviewheight
                << p.bob << p.pitch << p.momx << p.momy << p.health << p.armorpoints << p.armorfactor << p.armordivisor
                << p.totalfrags << p.extralight << p.cheats << p.refire << p.killcount << p.itemcount << p.secretcount
                << p.didsecret << p.damagecount << p.bonuscount << p.fixedcolormap << p.colormap << p.quake
                << p.jumptime << p.inv_ptr;
...
static void P_ArchiveWorld(SaveArchive &arc)
{
    ...
    for(i = 0, sec = sectors; i < numsectors; ++i, ++sec)
    {
        ...
        if(arc.saveVersion() <= 17 && arc.isLoading())
        {
            ...
            sec->flags =
                (sec->flags & (SECF_MONSTERDEATH - 1)) | ((sec->flags & ~((unsigned)SECF_MONSTERDEATH - 1)) << 1);
        }
        else if(arc.saveVersion() <= 11 && arc.isLoading())
        {
            // Adjust for the INSTANTDEATH flag inserted in the middle
            sec->flags =
                (sec->flags & (SECF_INSTANTDEATH - 1)) | ((sec->flags & ~((unsigned)SECF_INSTANTDEATH - 1)) << 1);
        }
        arc << sec->intflags << sec->damage << sec->damageflags << sec->leakiness << sec->damagemask << sec->damagemod;
        Archive_Flat(arc, floor.pic);
        Archive_Flat(arc, ceiling.pic);
        arc << sec->lightlevel << sec->oldlightlevel << floor.lightdelta << ceiling.lightdelta << sec->special
            << sec->tag; // needed?   yes -- transfer types -- killough
        if(arc.saveVersion() >= 10)
        {
            ...
            arc << floor.offset << ceiling.offset << floor.angle << ceiling.angle << sec->soundtraversed
                << sec->stairlock << sec->prevsec << sec->nextsec;
    ...
    for(i = 0, li = lines; i < numlinesSave; ++i, ++li)
    {
        ...
        arc << li->flags << li->special << li->tag << li->args[0] << li->args[1] << li->args[2] << li->args[3]
            << li->args[4];
        ...
        for(j = 0; j < 2; j++)
        {
            if(li->sidenum[j] != -1)
            {
                ...
                {
                    arc << si->offset_top_x << si->offset_top_y << si->offset_bottom_x << si->offset_bottom_y
...
{
    arc << LevelInfo.airControl << LevelInfo.airFriction << LevelInfo.gravity << LevelInfo.skyDelta
        << LevelInfo.sky2Delta;
}
    ...
    else
    {
        ...
        if(markpointnum)
        {
            while(markpointnum >= markpointnum_max)
            {
                ...
                markpoints       = erealloc(markpoint_t *, markpoints, sizeof *markpoints * markpointnum_max);
            }
...
static void P_UnArchiveSndSeq(SaveArchive &arc)
{
    ...
    arc << newSeq->delayCounter << newSeq->volume << newSeq->attenuation << newSeq->flags;
...
static void P_ArchiveButtons(SaveArchive &arc)
{
    ...
    for(int i = 0; i < numsaved; i++)
    {
        ...
        arc << buttonlist[i].btimer << buttonlist[i].dopopout << buttonlist[i].line << buttonlist[i].side
            << buttonlist[i].where << buttonlist[i].switchindex;
    }
}
