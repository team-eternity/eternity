...
static bool S_CheckSectorKill(const sector_t *earsec, const PointThinker *src)
{
    ...
    if(gamestate == GS_LEVEL)
    {
        ...
        if(src && R_PointInSubsector(src->x, src->y)->sector->flags & SECF_KILLSOUND)
            return true;
    }
...
//
static int S_AdjustSoundParams(camera_t *listener, const PointThinker *source, int chanvol, int chanattn, int *vol,
                               int *sep, int *pitch, int *pri, sfxinfo_t *sfx)
{
    ...
    dist = adx ? FixedDiv(adx, finesine[(tantoangle_acc[FixedDiv(ady, adx) >> DBITS] + ANG90) >> ANGLETOFINESHIFT]) : 0;
    ...
    *sep = NORM_SEP - FixedMul(S_STEREO_SWING >> FRACBITS, finesine[angle]);
    ...
    // volume calculation
    *vol = dist < close_dist >> FRACBITS ? basevolume : basevolume * ((clipping_dist >> FRACBITS) - dist) / attenuator;
...
//
static int S_getChannel(const PointThinker *origin, sfxinfo_t *sfxinfo, int priority, int singularity, int schan,
                        bool nocutoff)
{
    ...
    else
    {
        ...
        for(cnum = 0; cnum < numChannels; cnum++)
        {
            ...
            if(channels[cnum].sfxinfo && channels[cnum].singularity == singularity && origin_equivalent &&
...
void S_StartSfxInfo(const soundparams_t &params)
{
    ...
    if((mo = thinker_cast<const Mobj *>(origin)))
    {
        ...
        if(mo == players[displayplayer].mo || (mo->flags & MF_MISSILE && mo->target == players[displayplayer].mo))
        {
            priority_boost = true;
        }
    }
    ...
    {
        // use an external cam?
        if(!S_AdjustSoundParams(listener, origin, volumeScale, params.attenuation, &volume, &sep, &pitch, &priority,
                                sfx))
            return;
...
static void S_StartSoundAtVolumeLooped(const PointThinker *origin, int sfx_id, int volume, int attn, int subchannel)
{
    soundparams_t params;
...
void S_StopSoundId(const PointThinker *origin, int sound_id)
{
    ...
    {
        if(channels[cnum].sfxinfo && channels[cnum].origin == origin &&
           channels[cnum].idnum == I_SoundID(channels[cnum].handle) && channels[cnum].aliasinfo &&
           channels[cnum].aliasinfo->dehackednum == sound_id)
        {
            S_StopChannel(cnum);
        }
    }
    ...
    for(int cnum = 0; cnum < numChannels; cnum++)
    {
        ...
        if(I_SoundIsPlaying(c->handle))
        {
            ...
            else if(c->origin && static_cast<const PointThinker *>(listener) != c->origin) // killough 3/20/98
            {
                ...
                if(!S_AdjustSoundParams(listener ? &playercam : nullptr, c->origin, c->volume, c->attenuation, &volume,
                                        &sep, &pitch, &pri, sfx))
                {
                    S_StopChannel(cnum);
                }
...
void S_ChangeMusicNum(int musnum, int looping)
{
    ...
    if(s_randmusic && musnum >= GameModeInfo->randMusMin && musnum <= GameModeInfo->randMusMax)
    {
...
musicinfo_t *S_MusicForName(const char *name)
{
    ...
    if(prefixlen > 0 && namelen > prefixlen && tempname.findSubStr(GameModeInfo->musPrefix) == tempname.constPtr())
    {
