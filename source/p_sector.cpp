...
void P_SaveSectorPositions()
{
    for(int i = 0; i < numsectors; i++)
    {
        ...
        si.prevheight.floor    = sec.srf.floor.height;
        si.prevheightf.floor   = sec.srf.floor.heightf;
        si.prevheight.ceiling  = sec.srf.ceiling.height;
        si.prevheightf.ceiling = sec.srf.ceiling.heightf;

        if(sec.srf.floor.slope)
            si.prevslopezf.floor = sec.srf.floor.slope->of.z;
        if(sec.srf.ceiling.slope)
            si.prevslopezf.ceiling = sec.srf.ceiling.slope->of.z;
    }
}
