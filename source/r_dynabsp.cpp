...
static dynaseg_t *R_selectPartition(dseglist_t segs)
{
    ...
    for(rover = segs; rover; rover = rover->dllNext)
    {
        ...
        for(crover = segs; crover; crover = crover->dllNext)
        {
            ...
            else if(a <= 0 && (!nearzero(a) || (nearzero(b) && check->pdx * part->pdx + check->pdy * part->pdy < 0)))
            {
            leftside:
                diff -= 2;
            }
...
//
void R_ComputeIntersection(const dynaseg_t *part, const dynaseg_t *seg, double &outx, double &outy, v2float_t *fbackup)
{
    double a2, b2, l2, w, d;
    ...
    if(d != 0.0)
    {
        ...
        if(fbackup)
        {
            ...
            else
            {
                ...
                {
                    const double bw = (dx * (segv1->fbackup.y - part->psy) + dy * (part->psx - segv1->fbackup.x)) / bd;
...
//
static void R_divideSegs(rpolynode_t *rpn, dseglist_t *ts, dseglist_t *rs, dseglist_t *ls)
{
