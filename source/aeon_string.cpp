...
#define MARGSIG(name)  "String &" name "(String &inout) const"
    ...
    void ScriptObjString::Init()
    {
        ...
        for(const aeonbehaviorreg_t &behavior : stringBehaviors)
            e->RegisterObjectBehaviour("String", behavior.behavior, behavior.declaration, behavior.funcPointer,
                                       asCALL_GENERIC);
        ...
        e->RegisterGlobalFunction("void print(const String &in)", WRAP_FN_PR(asPrint, (const qstring &), void),
                                  asCALL_GENERIC);
    }
