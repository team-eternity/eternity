...
static void MN_HMainMenuDrawer()
{
    ...
    // draw M_HTIC
    V_DrawPatch(88, 0, &subscreen43, PatchLoader::CacheName(wGlobalDir, "M_HTIC", PU_CACHE));

    // draw spinning skulls
    skullIndex = (menutime / 3) % NUM_HSKULL;

    V_DrawPatch(40, 10, &subscreen43, PatchLoader::CacheNum(wGlobalDir, HSkullLumpNums[17 - skullIndex], PU_CACHE));

    V_DrawPatch(232, 10, &subscreen43, PatchLoader::CacheNum(wGlobalDir, HSkullLumpNums[skullIndex], PU_CACHE));
}
