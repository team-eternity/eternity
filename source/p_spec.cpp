...
static void P_addPicAnim(const animdef_t &animdef)
{
    ...
    {
        // different episode ?
        if(R_CheckForWall(animdef.startname) == -1 || R_CheckForWall(animdef.endname) == -1)
            return;
    ...
    {
        if(R_CheckForFlat(animdef.startname) == -1 || R_CheckForFlat(animdef.endname) == -1)
            return;
...
static void P_applyEDFAnim(const EAnimDef &ead)
{
    ...
    {
        if(anim->basepic == startpic && anim->istexture == (ead.type == EAnimDef::type_wall))
        {
            break;
        }
    }
...
void P_InitPicAnims(void)
{
    ...
    {
        if(E_IsHexenAnimation(animdefs[i].startname, animdefs[i].istexture ? EAnimDef::type_wall : EAnimDef::type_flat))
        {
...
int twoSided(int sector, int line)
{
    ...
    return getComp(comp_model) ? sectors[sector].lines[line]->flags & ML_TWOSIDED :
...
sector_t *getNextSector(const line_t *line, const sector_t *sec)
{
    ...
    return getComp(comp_model) && !(line->flags & ML_TWOSIDED) ? nullptr :
           line->frontsector == sec ? getComp(comp_model) || line->backsector != sec ? line->backsector : nullptr :
                                      line->frontsector;
}
                              ...
                              const fixed_t &(*comp)(const fixed_t &, const fixed_t &))
{
    ...
    return comp(sector.srf[surf].getZAt(line.v1->x, line.v1->y), sector.srf[surf].getZAt(line.v2->x, line.v2->y));
}
        ...
        if(twoSided(secnum, i))
        {
            ...
            if(originSec.srf.floor.slope)
            {
                if(sec->srf.floor.slope &&
                   P_SlopesEqualAtGivenHeight(*originSec.srf.floor.slope, floordestheight, *sec->srf.floor.slope))
                {
                    return sec;
                }
            }
        ...
        if(twoSided(secnum, i))
        {
            ...
            if(originSec.srf.ceiling.slope)
            {
                if(sec->srf.ceiling.slope &&
                   P_SlopesEqualAtGivenHeight(*originSec.srf.ceiling.slope, ceildestheight, *sec->srf.ceiling.slope))
                {
                    return sec;
                }
            }
...
{
    start = (start >= 0 ? lines[start].nexttag : lines[(unsigned int)tag % (unsigned int)numlines].firsttag);
...
{
    start = (start >= 0 ? sectors[start].nexttag : sectors[(unsigned int)tag % (unsigned int)numsectors].firsttag);
...
static void P_InitTagLists()
{
    ...
    {                            // so that lower linedefs appear first
        ...
        if(LevelInfo.mapFormat == LEVEL_FORMAT_DOOM || LevelInfo.mapFormat == LEVEL_FORMAT_PSX || lines[i].tag != -1)
        {
...
static void P_runInstantDeathSector(player_t *player, const sector_t *sector)
{
    ...
    {
        kill = sector->damageflags & SDMG_IGNORESUIT ||
               (!player->powers[pw_ironfeet].isActive() && !player->powers[pw_invulnerability].isActive());
    }
    if(!kill)
        return;
...
void P_PlayerInSpecialSector(player_t *player, sector_t *sector)
{
    ...
    else if(enable_nuke && sector->damage > 0) // killough 12/98: nukage disabling cheat
    {
            ...
            (P_Random(pr_slimehurt) < sector->leakiness)))
        {
            ...
            {
                // do the damage
                P_DamageMobj(player->mo, nullptr, nullptr, sector->damage, sector->damagemod);
...
void P_PlayerOnSpecialFlat(const player_t *player)
{
    ...
    {
        P_DamageMobj(player->mo, nullptr, nullptr, terrain->damageamount, terrain->damagetype);
...
void P_UpdateSpecials()
{
    ...
    for(anim = anims; anim < lastanim; anim++)
    {
        for(int i = anim->basepic; i < anim->basepic + anim->numpics; i++)
        {
            ...
            if((i >= flatstart && i < flatstop && r_swirl) || anim->speed >= SWIRL_TICS || anim->numpics == 1)
            {
...
// Namely, colormaps.
//
static void P_SetupHeightTransfer(int linenum, int secnum, const UDMFSetupSettings &setupSettings)
{
    int       s;
...
void P_SpawnSpecials(UDMFSetupSettings &setupSettings)
{
    ...
    for(int i = 0; i < numlines; i++)
    {
        ...
        case EV_STATIC_EXTRADATA_SECTOR: E_LoadSectorExt(&lines[i], setupSettings); break;
    ...
    for(int i = 0; i < numsectors; i++)
    {
        ...
        for(int j = 0; j < sec->linecount; ++j)
        {
            ...
            if(line->pflags & PS_PASSABLE)
                foundportal = true;
            if(line->backsector && (!(line->pflags & PS_PASSABLE) || line->frontsector == sec))
            {
                isbox = false;
                break;
            }
...
void FrictionThinker::Think()
{
    ...
    while(node)
    {
        ...
        thing = node->m_thing;
        if(thing->player && !(thing->flags & (MF_NOGRAVITY | MF_NOCLIP)) && P_RestingOnGround(*thing, sec->srf.floor))
        {
...
static void P_SpawnFriction()
{
    ...
    for(i = 0; i < numlines; i++, line++)
    {
        ...
        {
            int length = line->args[1] ? line->args[1] : P_AproxDistance(line->dx, line->dy) >> FRACBITS;
            int friction;
            int movefactor, s;
...
line_t *P_FindLine(int tag, int *searchPosition, line_t *defaultLine)
{
    ...
    int start = (*searchPosition >= 0 ? lines[*searchPosition].nexttag :
...
//
static void P_addLineToAttachList(const line_t *line, int *&attached, int &numattach, int &maxattach)
{
    if(!line->frontsector || !line->backsector || !(line->flags & ML_3DMIDTEX))
    {
        return;
    }
    ...
    for(int i = 0; i < numsectors; ++i)
    {
        ...
        for(int j = 0; j < numsectors; ++j)
        {
            ...
            if(attach.floorid)
            {
                if(abs(slave.attachfloor) == attach.floorid)
                {
                    if(j == i)
                        continue;
                    found   = false;
                    settype = (slave.attachfloor ^ attach.floorid) >= 0 ? AS_FLOOR : AS_MIRRORFLOOR;
                ...
                {
                    found   = false;
                    settype = (slave.attachceiling ^ attach.floorid) >= 0 ? AS_CEILING : AS_MIRRORCEILING;
            ...
            if(attach.ceilingid)
            {
                ...
                {
                    found   = false;
                    settype = (slave.attachfloor ^ attach.ceilingid) >= 0 ? AS_FLOOR : AS_MIRRORFLOOR;
                ...
                if(abs(slave.attachceiling) == attach.ceilingid)
                {
                    if(j == i)
                        continue;
                    found   = false;
                    settype = (slave.attachceiling ^ attach.ceilingid) >= 0 ? AS_CEILING : AS_MIRRORCEILING;
...
void P_AttachSectors(const line_t *line, int staticFn)
{
    ...
    if(!ceiling && sector->srf.floor.asurfacecount)
    {
        ...
        if(numattached >= maxattached)
        {
            ...
            attached    = erealloc(attachedsurface_t *, attached, sizeof(attachedsurface_t) * maxattached);
        }
    ...
    for(start = -1; (start = P_FindLineFromLineArg0(line, start)) >= 0;)
    {
        ...
        if(start != line - lines)
        {
            ...
            if(slavefunc == EV_STATIC_ATTACH_FLOOR_TO_CONTROL)
            {
                ...
                if(slaveline->frontsector == sector && staticFn == EV_STATIC_ATTACH_SET_FLOOR_CONTROL)
                    continue;
            ...
            else if(slavefunc == EV_STATIC_ATTACH_CEILING_TO_CONTROL)
            {
                ...
                if(slaveline->frontsector == sector && staticFn == EV_STATIC_ATTACH_SET_CEILING_CONTROL)
                    continue;
    ...
    if(ceiling)
    {
        ...
        sector->srf.ceiling.asurfaces =
            emalloctag(attachedsurface_t *, sizeof(attachedsurface_t) * numattached, PU_LEVEL, nullptr);
...
// where.
//
static bool P_getParamPortalProps(const int *args, portal_type &type, portal_effect &effects)
{
    ...
    default:
        ...
        type = args[paramPortal_argPlane] == paramPortal_planeBoth ? portal_anchored : portal_twoway;
...
static int P_findParamPortalAnchor(const line_t *line)
{
    ...
    {
        if(lines[s].special != anchortype || line == &line[s] || lines[s].args[0] != line->args[0] ||
...
static void P_SpawnPortal(line_t *line, int staticFn)
{
    ...
    case portal_anchored:
        ...
        {
            if(staticFn == EV_STATIC_PORTAL_ANCHORED_CEILING || staticFn == EV_STATIC_PORTAL_ANCHORED_CEILING_FLOOR)
    ...
    case portal_linked:
        ...
        if(param)
        {
            if(staticFn == EV_STATIC_PORTAL_SECTOR_PARAM_COMPAT)
            {
                planez = line->args[2] == 0 ? sector->srf.ceiling.height : sector->srf.floor.height;
                s      = P_findParamPortalAnchor(line);
            }
            else // line portal
            {
                ...
                if(line->args[ev_LinePortal_Arg_Type] == ev_LinePortal_Type_EEClassic)
                {
                    ...
                    {
                        if(lines[s].special != anchortype || line == &lines[s] || !lines[s].frontsector ||
                           lines[s].args[ev_LinePortal_Arg_Type] != ev_LinePortal_Type_EEClassic ||
        ...
        else
        {
            ...
            for(s = -1; (s = P_FindLineFromTag(tag, s)) >= 0;)
            {
                ...
                if((lines[s].special != anchortype &&
                    (anchorfunc != EV_STATIC_PORTAL_LINKED_L2L_ANCHOR ||
                     !(lines[s].extflags & (EX_ML_LOWERPORTAL | EX_ML_UPPERPORTAL)))) ||
           ...
           line->args[ev_LinePortal_Arg_Type] != ev_LinePortal_Type_EEClassic)
        {
            ...
            if(lines[s].portal && lines[s].portal->type == R_LINKED && (!line->backsector || !lines[s].backsector))
            {
        ...
        otherIsEdge = !!(lines[s].extflags & (EX_ML_LOWERPORTAL | EX_ML_UPPERPORTAL));

        // Special case where the portal was created with the line-to-line portal type
        if(staticFn == EV_STATIC_PORTAL_LINKED_LINE2LINE || staticFn == EV_STATIC_PORTAL_LINE_PARAM_COMPAT)
        {
            if(!otherIsEdge)
            {
    ...
    for(s = -1; (s = P_FindLineFromLineArg0(line, s)) >= 0;)
    {
        ...
        {
            if((lines[s].intflags & MLI_FLOORPORTALCOPIED && effects == portal_ceiling) ||
               (lines[s].intflags & MLI_CEILINGPORTALCOPIED && effects == portal_floor))
            {
        ...
        else if(xferfunc == EV_STATIC_PORTAL_APPLY_FRONTSECTOR)
        {
            ...
            if(lines[s].intflags & MLI_CEILINGPORTALCOPIED && lines[s].intflags & MLI_FLOORPORTALCOPIED)
            {
                lines[s].intflags &= ~(MLI_FLOORPORTALCOPIED | MLI_CEILINGPORTALCOPIED);
            }
        }
        else
            continue;
