...
// type.
//
void S_StartSequenceNum(PointThinker *mo, int seqnum, int seqtype, int seqOriginType, int seqOriginIdx)
{
    ESoundSeq_t *edfSeq;
    SndSeq_t    *newSeq;
    ...
    newSeq->volume = edfSeq->randvol ? M_RangeRandom(edfSeq->minvolume, edfSeq->volume) : edfSeq->volume;
}
...
void S_StartSectorSequence(sector_t *s, int seqtype)
{
    ...
    S_StartSequenceNum(SECTOR_ORIGIN(s, origintype), s->sndSeqID, seqtype, origintype, int(s - sectors));
}
...
{
    S_StartSequenceNum(&po->spawnSpot, po->seqId, SEQ_DOOR, SEQ_ORIGIN_POLYOBJ, po->id);
}
...
//
void S_StartSequenceName(PointThinker *mo, const char *seqname, int seqOriginType, int seqOriginIdx)
{
    ESoundSeq_t *edfSeq;
    SndSeq_t    *newSeq;
...
{
    S_StartSequenceName(SECTOR_ORIGIN(s, originType), seqname, originType, int(s - sectors));
}
...
static void S_RunSequence(SndSeq_t *curSeq)
{
    ...
    case SEQ_CMD_DELAYRANDOM: // delay a random amount within given range
        curSeq->currentSound = nullptr;
        {
            ...
            curSeq->delayCounter =
                vanilla_heretic ? min + (P_Random(pr_envirotics) % (max - min + 1)) : (int)M_RangeRandomEx(min, max);
...
static void S_RunEnviroSequence()
{
    ...
    else if(nextEnviroSpot)
    {
        ...
        enviroSeq.volume = edfSeq->randvol ? vanilla_heretic ? P_Random(pr_envirospot) / 4 :
                                                               M_RangeRandom(edfSeq->minvolume, edfSeq->volume) :
                                             edfSeq->volume;
