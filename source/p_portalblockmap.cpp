...
void PortalBlockmap::mapInit()
{
    ...
    for(int i = 0; i < bmapwidth * bmapheight; ++i)
    {
        ...
        const sector_t &sector = *R_PointInSubsector(bmaporgx + i % bmapwidth * MAPBLOCKSIZE + MAPBLOCKSIZE / 2,
                                                     bmaporgy + i / bmapwidth * MAPBLOCKSIZE + MAPBLOCKSIZE / 2)
                                      ->sector;
...
void PortalBlockmap::linkLine(const line_t &line)
{
    ...
    int diff =
        !adx ? 1 :
        !ady ? -1 :
               (((x >> MAPBTOFRAC) << MAPBTOFRAC) + (dx > 0 ? MAPBLOCKUNITS - 1 : 0) - x) * (ady = D_abs(ady)) * dx -
                   (((y >> MAPBTOFRAC) << MAPBTOFRAC) + (dy > 0 ? MAPBLOCKUNITS - 1 : 0) - y) * (adx = D_abs(adx)) * dy;
    ...
    // ending block
    const int bend =
        (((line.v2->y / FRACUNIT) - miny) >> MAPBTOFRAC) * bmapwidth + (((line.v2->x / FRACUNIT) - minx) >> MAPBTOFRAC);
...
//
void PortalBlockmap::checkLinkSector(const sector_t &sector, const portal_t *portal, surf_e surf, int mapindex)
{
    ...
    {
        if(entry.type == portalblocktype_e::sector && entry.sector == &sector && entry.ldata == &portal->data.link)
        {
...
bool P_BlockHasLinkedPortals(int index, bool includesectors)
{
    ...
    if(index < 0 || index >= bmapheight * bmapwidth)
        return false;
    if(portalmap[index] & (PMF_LINE | (includesectors ? PMF_FLOOR | PMF_CEILING : 0)))
    {
        return true;
    }

    for(const DLListItem<polymaplink_t> *plink = polyblocklinks[index]; plink; plink = plink->dllNext)
    {
