...
namespace Aeon
{
    ...
    {
        return P_SpawnMissileAngle(source, E_SafeThingName(type.constPtr()), angle, momz, z);
    }
    ...
    static void lineAttack(Mobj *t1, angle_t angle, fixed_t distance, fixed_t slope, int damage,
                           const qstring &pufftype)
    {
...
#define STATE_GET_SIG(name) "const EE::State @const get_" name "() const property"

    static const aeonfuncreg_t mobjinfoFuncs[] = {
        { STATE_GET_SIG("spawnstate"),                 WRAP_OBJ_LAST(getState<&mobjinfo_t::spawnstate>)    },
        { STATE_GET_SIG("seestate"),                   WRAP_OBJ_LAST(getState<&mobjinfo_t::seestate>)      },
        { STATE_GET_SIG("painstate"),                  WRAP_OBJ_LAST(getState<&mobjinfo_t::painstate>)     },
        { STATE_GET_SIG("meleestate"),                 WRAP_OBJ_LAST(getState<&mobjinfo_t::meleestate>)    },
        { STATE_GET_SIG("missilestate"),               WRAP_OBJ_LAST(getState<&mobjinfo_t::missilestate>)  },
        { STATE_GET_SIG("deathstate"),                 WRAP_OBJ_LAST(getState<&mobjinfo_t::deathstate>)    },
        { STATE_GET_SIG("xdeathstate"),                WRAP_OBJ_LAST(getState<&mobjinfo_t::xdeathstate>)   },
        { STATE_GET_SIG("raisestate"),                 WRAP_OBJ_LAST(getState<&mobjinfo_t::raisestate>)    },
        { STATE_GET_SIG("crashstate"),                 WRAP_OBJ_LAST(getState<&mobjinfo_t::crashstate>)    },
        { STATE_GET_SIG("activestate"),                WRAP_OBJ_LAST(getState<&mobjinfo_t::activestate>)   },
        { STATE_GET_SIG("inactivestate"),              WRAP_OBJ_LAST(getState<&mobjinfo_t::inactivestate>) },
    ...
    void ScriptObjMobj::Init()
    {
        ...
        for(const aeonbehaviorreg_t &behavior : mobjBehaviors)
            e->RegisterObjectBehaviour("Mobj", behavior.behavior, behavior.declaration, behavior.funcPointer,
                                       asCALL_GENERIC);
        ...
        e->RegisterGlobalFunction("fixed_t FloatBobOffsets(const int index)", WRAP_FN(floatBobOffsets), asCALL_GENERIC);
