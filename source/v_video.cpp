...
static void V_scaleRect(vrect_t *r, VBuffer *buffer)
{
    ...
    if(r->sx < 0 || r->sx + r->sw > buffer->width || r->sy < 0 || r->sy + r->sh > buffer->height)
    {
...
void V_CopyRect(int srcx, int srcy, VBuffer *src, int width, int height, int destx, int desty, VBuffer *dest)
{
    ...
    if(src->scaled != dest->scaled || src->ixscale != dest->ixscale || src->iyscale != dest->iyscale)
    {
    ...
    if(srcx + width < 0 || srcy + height < 0 || srcx >= src->unscaledw || srcy >= src->unscaledh)
        return;

    // quick rejection if dest rect is off-screen
    if(destx + width < 0 || desty + height < 0 || destx >= dest->unscaledw || desty >= dest->unscaledh)
        return;
...
// lighting remapping.
//
void V_DrawPatchTranslatedLit(int x, int y, VBuffer *buffer, patch_t *patch, byte *outr, byte *lighttable, bool flipped)
{
    PatchInfo         pi;
    cb_patch_column_t patchcol{};
...
void V_InitFlexTranTable(const byte *palette)
{
    ...
    for(r = 0; r < 32; ++r)
    {
        for(g = 0; g < 32; ++g)
        {
            for(b = 0; b < 32; ++b)
            {
                RGB32k[r][g][b] = V_FindBestColor(palette, MAKECOLOR(r), MAKECOLOR(g), MAKECOLOR(b));
            }
        }
    }
    ...
    for(x = 0; x < 65; ++x)
    {
        for(y = 0; y < 256; ++y)
        {
            Col2RGB8[x][y] = (((tempRGBpal[y].r * x) >> 4) << 20) | ((tempRGBpal[y].g * x) >> 4) |
