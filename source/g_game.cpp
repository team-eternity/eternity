...
void G_Ticker(void)
{
   ...
   if((paused & 2) || (!demoplayback && (menuactive || consoleactive) && !netgame && !clientserver))
   {
...
Mobj* G_SpawnFog(fixed_t x, fixed_t y, angle_t angle)
{
   ...
   else
   {
      ...
      angle_t mtangle = (angle_t)(angle / 45);
...
void G_ReloadDefaults(void)
{
   ...
   // [CG] These things are handled elsewhere in c/s.
   if(clientserver)
   {
      CS_ReloadDefaults();
      return;
