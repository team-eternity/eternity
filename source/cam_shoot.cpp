...
//
bool ShootContext::shotCheck2SLine(line_t *li, int lineside, fixed_t dist, v2fixed_t edgepos) const
{
    bool ret = false;
    ...
    if(in->isaline)
    {
        ...
        if(context.shotCheck2SLine(li, lineside, dist, edgepos))
        {
            ...
            if(portal && lineside == 0 && in->frac > 0)
            {
                ...
                v.z = context.state.v.z +
                      FixedMul(context.params.aimslope, FixedMul(in->frac, context.params.attackrange));
                fixed_t dist    = FixedMul(context.params.attackrange, in->frac);
        ...
        fixed_t frac = in->frac - FixedDiv(4 * FRACUNIT, context.params.attackrange + context.state.origindist);
        ...
        if(sidesector)
        {
            ...
            if(!P_CheckShootPlane(*sidesector, trace.x, trace.y, context.state.v.z, context.params.aimslope,
                                  context.prevedgepos, context.prevfrac, context.params.attackrange, context.cos,
                                  context.sin, x, y, z, hitplane, updown))
            {
                return false;
            }
        }
        ...
        P_SpawnPuff(x, y, z, P_PointToAngle(0, 0, li->dx, li->dy) - ANG90, updown, true, context.params.thing,
                    E_PuffForIndex(context.params.puffidx));

        return false;
    }
    return P_ShootThing(in, context.params.thing, context.params.attackrange, context.state.v.z,
                        context.params.aimslope, context.params.attackrange + context.state.origindist, trace,
                        context.params.puffidx, context.params.damage);
}

//
// ShootContext::ShootContext
//
ShootContext::ShootContext(const params_t &params, const State *instate) : params(params)
{
    ...
    else
    {
        ...
        state.v.z        = params.thing->z - params.thing->floorclip + (params.thing->height >> 1) + 8 * FRACUNIT;
