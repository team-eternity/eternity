...
static EHashTable<EAnimDef, ENCQStrHashKey, &EAnimDef::startpic, &EAnimDef::link> e_anim_namehash(NUMANIMCHAINS);
...
static void E_processAnimation(cfg_t *cfg)
{
    ...
    if(seqsize >= 1)
    {
        ...
        for(unsigned i = 0; i < seqsize; ++i)
        {
            ...
            newpic.flags = E_ParseFlags(cfg_getstr(pic, ITEM_PIC_FLAGS), &anim_flagset);
        }
        return;
    }
    // Doom style
