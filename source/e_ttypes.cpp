...
cfg_opt_t edf_splash_opts[] = { COMMON_SPLASH_OPTIONS, CFG_END() };

cfg_opt_t edf_spldelta_opts[] = { CFG_STR(ITEM_SPLASHDELTA_NAME, nullptr, CFGF_NONE), COMMON_SPLASH_OPTIONS,
                                  CFG_END() };
...
static void E_ProcessSplash(cfg_t *const cfg, const bool def)
{
    ...
    const auto IS_SET = [cfg, def](const char *const name) -> bool { return def || cfg_size(cfg, (name)) > 0; };
    ...
    E_EDFLogPrintf("\t\t\t%s splash '%s'\n", newsp ? "Finished" : "Modified", newSplash->name);
}
...
static void E_ProcessTerrain(cfg_t *const cfg, const bool def)
{
    ...
    else
    {
        ...
        if(!tempstr)
        {
            E_EDFLoggedErr(3, "E_ProcessTerrain: terrain delta requires name field!\n");
        }
    ...
    {
        newTerrain->footclip = cfg_getint(cfg, ITEM_TERRAIN_FOOTCLIP) * FRACUNIT;
    }
    ...
    if(def)
    {
        E_EDFLogPrintf("\t\t\t%s terrain '%s'\n", newtr ? "Finished" : "Modified", newTerrain->name);
    }
    else
...
ETerrain *E_GetTerrainTypeForPt(fixed_t x, fixed_t y, int position)
{
    ...
    switch(position)
    {
    case 0:  return floorterrain;
    case 1:  return ceilingterrain;
...
//
static void E_TerrainHit(const ETerrain *terrain, Mobj *thing, fixed_t z, const sector_t *sector, fixed_t splashAlpha)
{
    ...
    S_StartSoundName(mo ? mo : thing, lowmass ? splash->smallsound : splash->sound);
}

//
// Get the information for the thing's floor terrain. Also gets the resulting z value
//
static const ETerrain &E_getFloorTerrain(const Mobj &thing, const sector_t &sector, v2fixed_t pos, fixed_t *z,
                                         fixed_t *splashAlpha)
{
    // override with sector terrain if one is specified
...
bool E_HitWater(Mobj *thing, const sector_t *sector)
{
    fixed_t         z;
    fixed_t         splashAlpha;
    const ETerrain &terrain = E_getFloorTerrain(*thing, *sector, { thing->x, thing->y }, &z, &splashAlpha);
...
void E_ExplosionHitWater(Mobj *thing, int damage)
{
    ...
    if(vanilla_heretic && thing->flags & MF_MISSILE && !(thing->flags2 & MF2_NOSPLASH) && E_WouldHitFloorWater(*thing))
    {
        return;
    }
    ...
    if(hit)
    {
        E_HitWater(thing, overlaySplashSector ? overlaySplashSector : P_ExtremeSectorAtPoint(thing, surf_floor));
    }
}
