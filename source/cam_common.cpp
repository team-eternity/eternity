...
{
    if(def.earlyOut != PTDef::eo_always && (x < 0 || y < 0 || x >= ::bmapwidth || y >= ::bmapheight))
    {
        return true;
    }
...
bool PathTraverser::checkLine(size_t linenum)
{
    ...
    s2 = P_PointOnDivlineSidePrecise(trace.x + trace.dx, trace.y + trace.dy, &dl);
    if(s1 == s2)
        return true; // line isn't crossed
...
bool PathTraverser::traverse(fixed_t cx, fixed_t cy, fixed_t tx, fixed_t ty)
{
    ...
    if(def.earlyOut == PTDef::eo_always && (xt1 < 0 || yt1 < 0 || xt1 >= bmapwidth || yt1 >= bmapheight || xt2 < 0 ||
    ...
    for(int count = 0; count < 100; count++)
    {
        ...
        if(!(def.flags & CAM_REQUIRELINEPORTALS) || P_BlockHasLinkedPortals(mapy * ::bmapwidth + mapx, false))
        {
        ...
        case 3:
            ...
            if(!(def.flags & CAM_REQUIRELINEPORTALS) || P_BlockHasLinkedPortals(mapy * ::bmapwidth + mapx, false))
            {
                if(def.flags & CAM_ADDLINES &&
                   (!blockLinesIterator(mapx + mapxstep, mapy) || !blockLinesIterator(mapx, mapy + mapystep)))
                {
                    return false;
                }
                // ioanch 20151230: autoaim support
                if(def.flags & CAM_ADDTHINGS &&
                   (!blockThingsIterator(mapx + mapxstep, mapy) || !blockThingsIterator(mapx, mapy + mapystep)))
                {
                    return false;
                }
            }
