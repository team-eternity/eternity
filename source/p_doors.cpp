...
//------------------------------------------------------------------------------
//
// Purpose: Door animation code (opening/closing).
    ...
    case plat_down:
        ...
        if(lighttag && topheight - bottomheight)
        {
            EV_LightTurnOnPartway(lighttag,
                                  FixedDiv(sector->srf.ceiling.height - bottomheight, topheight - bottomheight));
        }
...
void VerticalDoorThinker::serialize(SaveArchive &arc)
{
    ...
    arc << type << topheight << speed << direction << topwait << topcountdown << lighttag << turbo;
}
