...
//------------------------------------------------------------------------------
//
// Purpose: Plats (i.e. elevator platforms) code, raising/lowering.
        ...
        else // else handle reaching end of up stroke
        {
            if(res == pastdest) // end of stroke
            {
                ...
                case raiseToNearestAndChange:
                    ...
                    if((GameModeInfo->type == Game_Heretic && rnctype == PRNC_DEFAULT) || rnctype == PRNC_HERETIC)
                        break;
                case blazeDWUS:
                case downWaitUpStay:
                case raiseAndChange:
...
void PlatThinker::serialize(SaveArchive &arc)
{
    ...
    int temprnc = rnctype;
    arc << speed << low << high << wait << count << status << oldstatus << crush << tag << type << temprnc;
