...
public:
    ...
    static void useLines(const player_t *player, fixed_t x, fixed_t y, const State *instate);
...
//
void UseContext::useLines(const player_t *player, fixed_t x, fixed_t y, const State *state)
{
...
bool UseContext::useTraverse(const intercept_t *in, void *vcontext, const divline_t &trace)
{
    ...
    if(li->special && !(li->pflags & PS_PASSABLE))
    {
        ...
        P_UseSpecialLine(context->thing, li,
                         P_PointOnLineSidePrecise(context->thing->x + link->x, context->thing->y + link->y, li));
    ...
    if(portal && P_PointOnLineSidePrecise(trace.x, trace.y, li) == 0 && in->frac > 0)
    {
        ...
        if(newfromid == context->state.groupid || context->state.reclevel >= RECURSION_LIMIT)
        {
            return true;
        }
