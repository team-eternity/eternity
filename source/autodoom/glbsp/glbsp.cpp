...
// IOANCH: modified for AutoDoom direct data processing
...
//  }
  ...
  DisplaySetTitle("glBSP Build Progress");
   ...
   // IOANCH: changed file to display level
  file_msg = UtilFormat("%s", cur_info->input_file);
 
  DisplaySetBarText(2, file_msg);
   // IOANCH: just once
  DisplaySetBarLimit(2, 10);
  DisplaySetBar(2, 0);
  ...
  // loop over each level in the wad
   ...
   // IOANCH: only one level to handle
//  while (FindNextLevel())
//  {
    ret = HandleLevel();

    if (ret != GLBSP_E_OK)
      return GLBSP_E_ReadError;  // IOANCH: return if failing to handle level
