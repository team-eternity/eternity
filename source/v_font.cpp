...
void V_FontWriteTextEx(const vtextdraw_t &textdraw)
{
    ...
    cx = (*ch == TEXT_CONTROL_ABSCENTER) ? (screen->unscaledw - V_FontLineWidth(font, ch)) >> 1 : textdraw.x;
    cy = textdraw.y;
    ...
    while((c = *ch++))
    {
        ...
        if(c >= TEXT_COLOR_MIN)
        {
            ...
            default:
                if(font->color && !useAltMap) // not all fonts support translations
                {
                    ...
                    switch(c)
                    {
                    case TEXT_COLOR_NORMAL: colnum = font->colorNormal; break;
                    case TEXT_COLOR_HI:     colnum = font->colorHigh; break;
                    case TEXT_COLOR_ERROR:  colnum = font->colorError; break;
                    default:                colnum = c - 128; break;
                    }
        ...
        {
            cx  = (flags & VTXT_ABSCENTER) ? (screen->unscaledw - V_FontLineWidth(font, ch)) >> 1 : textdraw.x;
            cy += font->cy;
            continue;
        }
        ...
        if(font->linear && font->data)
        {
            ...
            V_DrawMaskedBlockTR(tx, cy, screen, font->lsize, font->lsize, font->lsize << 5, src, color);
        }
        else
        {
            // draw character
...
//
void V_FontWriteTextColored(vfont_t *font, const char *s, int color, int x, int y, VBuffer *screen)
{
