...
static bool PTR_linePortalCrossing(const intercept_t *in, void *vdata, const divline_t &trace)
{
    ...
    if(originside != 0 || originside == P_PointOnLineSidePrecise(trace.x + trace.dx, trace.y + trace.dy, line))
    {
        return true;
    }
...
//
v2fixed_t P_LinePortalCrossing(fixed_t x, fixed_t y, fixed_t dx, fixed_t dy, int *group, const line_t **passed)
{
    v2fixed_t cur = { x, y };
    v2fixed_t fin = { x + dx, y + dy };

    if((!dx && !dy) || full_demo_version < make_full_version(340, 48) || P_PortalGroupCount() <= 1)
    {
...
static bool PIT_exactTraverse(const line_t &line, void *vdata)
{
    ...
    int destside = P_PointOnLineSidePrecise(data.trace->x + data.trace->dx, data.trace->y + data.trace->dy, &line);
...
//
sector_t *P_ExtremeSectorAtPoint(fixed_t x, fixed_t y, surf_e surf, sector_t *sector, v2fixed_t *totaldelta)
{
    ...
    if(numgroups <= 1 || full_demo_version < make_full_version(340, 48) || !gMapHasSectorPortals ||
       sector->groupid == R_NOGROUP)
    {
    ...
    while(sector->srf[surf].pflags & PS_PASSABLE && loopprotection--)
    {
        ...
        if((!(sector->srf[surf].pflags & PF_ATTACHEDPORTAL) &&
            isInner(surf, sector->srf[surf].height + pSlopeHeights[sector - sectors].delta[surf], link.planez)) ||
           (!link.delta.x && !link.delta.y))
        {
            return sector;
        }
...
static bool P_boxTouchesBlockPortal(const portalblockentry_t &entry, const fixed_t bbox[4])
{
    ...
    if(!M_BoxesTouch(pSectorBoxes[&sector - sectors].box, bbox))
        return false;
    if(R_PointInSubsector(bbox[BOXLEFT] / 2 + bbox[BOXRIGHT] / 2, bbox[BOXBOTTOM] / 2 + bbox[BOXTOP] / 2)->sector ==
       &sector)
    {
        return true;
    }
        ...
        auto operate = [accessedgroupids, portalqueue, &queueback, func, &groupid, data, gcount,
            ...
            for(const portalblockentry_t &entry : block)
            {
                if(accessedgroupids[entry.ldata->toid])
                    continue;
                if(entry.type == portalblocktype_e::sector && !(entry.sector->srf[entry.surf].pflags & PS_PASSABLE))
                {
...
sector_t *P_PointReachesGroupVertically(fixed_t cx, fixed_t cy, fixed_t cmidz, int cgroupid, int tgroupid,
                                        sector_t *csector, fixed_t midzhint, surf_e *surf)
{
    ...
    if(!groupVisit)
    {
        groupVisit = ecalloctag(bool *, P_PortalGroupCount(), sizeof(*groupVisit), PU_LEVEL, (void **)&groupVisit);
    }
    else
