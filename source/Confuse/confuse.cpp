...
#else
#define cfg_assert(test) ((void)((test)||(my_assert(#test, __FILE__, __LINE__),0)))
#endif
...
cfg_opt_t *cfg_gettitleopt(cfg_t *cfg)
{
    ...
    {
        if(is_set(CFGF_TITLEPROPS, cfg->opts[i].flags) && cfg->opts[i].type == CFGT_MVPROP)
            return &cfg->opts[i];
    }
...
{
    opt->values = erealloc(cfg_value_t **, opt->values, (opt->nvalues + 1) * sizeof(cfg_value_t *));
    cfg_assert(opt->values);
...
cfg_value_t *cfg_setopt(cfg_t *cfg, cfg_opt_t *opt, const char *value)
{
    ...
    case CFGT_INT:
        ...
        else
        {
            ...
            if(errno == ERANGE)
            {
                cfg_error(cfg, "integer value '%s' for option '%s' is out of range\n", value, opt->name);
                return nullptr;
            }
        }
        break;
...
static void cfg_addlist_internal(cfg_t *cfg, cfg_opt_t *opt, unsigned int nvalues, va_list ap)
{
    unsigned int i;
...
static void cfg_addlistptr_internal(cfg_t *cfg, cfg_opt_t *opt, unsigned int nvalues, const void *values)
{
