...
//
void P_CreateMetaInfo(int map, const char *levelname, int par, const char *mus, int next, int secr, bool finale,
                      const char *intertext, int mission, const char *interpic)
{
    metainfo_t *mi;
...
static EHashTable<sndinfomus_t, EIntHashKey, &sndinfomus_t::mapnum, &sndinfomus_t::links> sndInfoMusHash(101);
...
static EHashTable<MusInfoMusic, ENCQStrHashKey, &MusInfoMusic::mapname, &MusInfoMusic::links> musInfoMusHash(101);
...
static const char *synthNames[SYNTH_NUMTYPES] = { "%s: New Level", "%s: Hidden Level" };
...
{
    if(M_IsExMy(gamemapname, nullptr, nullptr) && gameepisode > 0 && gameepisode <= GameModeInfo->numEpisodes &&
       gamemap > 0 && gamemap <= 9)
    {
...
void P_Doom2DefaultLevelName(levelnamedata_t &lnd)
{
    ...
    if(DEH_StringChanged("HUSTR_33") || (GameModeInfo->missionInfo->flags & MI_HASBETRAY))
        d2upperbound = 33;
...
void P_HticDefaultLevelName(levelnamedata_t &lnd)
{
    ...
    if(M_IsExMy(gamemapname, nullptr, nullptr) && gameepisode > 0 && gameepisode <= maxEpisode && gamemap > 0 &&
       gamemap <= 9)
    {
...
const finalerule_t *P_DetermineEpisodeFinaleRule(bool checkmap)
{
    ...
    for(const finalerule_t *rule = fdata->rules; rule->gameepisode != -2; rule++)
    {
        ...
        bool pickit = checkmap ? rule->gamemap == gamemap : rule->finaleType != FINALE_TEXT || rule->endOfGame;
        if((rule->gameepisode == -1 || rule->gameepisode == gameepisode) && (rule->gamemap == -1 || pickit))
        {
            return rule;
        }
    }
...
static void P_InfoDefaultFinale()
{
    ...
    {
        if(!(GameModeInfo->flags & GIF_SHAREWARE) && gameepisode >= GameModeInfo->numEpisodes)
...
{
    return LevelInfo.mapFormat == LEVEL_FORMAT_HEXEN && LevelInfo.levelType == LI_TYPE_HEXEN;
}
