...
static int EV_NullPostCrossLine(ev_action_t *action, int result, ev_instance_t *instance)
{
    return false;
}
...
// specials.
//
static int EV_DOOMPostCrossLine(ev_action_t *action, int result, ev_instance_t *instance)
{
    unsigned int flags = EV_CompositeActionFlags(action);
...
//
static int EV_DOOMPostUseLine(ev_action_t *action, int result, ev_instance_t *instance)
{
    unsigned int flags = EV_CompositeActionFlags(action);
...
// Post-activation semantics for DOOM-style use line actions.
//
static int EV_DOOMPostShootLine(ev_action_t *action, int result, ev_instance_t *instance)
{
    unsigned int flags = EV_CompositeActionFlags(action);
    ...
    if(instance->line && flags & EV_POSTCHANGESWITCH)
    {
        ...
        if(result || (flags & EV_POSTCHANGEALWAYS) || (action->minversion < 200 && EV_ClearSwitchOnFail()))
        {
            int useAgain   = !(flags & EV_POSTCLEARSPECIAL);
            int changeSide = (flags & EV_POSTCHANGESIDED) ? instance->side : 0;
            P_ChangeSwitchTexture(instance->line, useAgain, changeSide);
...
static bool EV_BOOMGenPreActivate(ev_action_t *action, ev_instance_t *instance)
{
    ...
    if(!EV_byPlayerOrALineEffect(*instance))
    {
        ...
        case GenTypeDoor:
            ...
            if(line && line->flags & ML_SECRET) // they can't open secret doors either
                return false;
            break;
        case GenTypeLocked: return false; // monsters disallowed from unlocking doors
        case GenTypeLift:
    ...
    case GenTypeCrusher:
        ...
        if((instance->genspac == WalkOnce || instance->genspac == WalkMany) && demo_version < 335)
            return false;
        break;
    default: //
        break;
...
//
static int EV_BOOMGenPostActivate(ev_action_t *action, int result, ev_instance_t *instance)
{
...
//
static int EV_ParamPostActivate(ev_action_t *action, int result, ev_instance_t *instance)
{
    ...
    if(result && instance->line)
    {
        ...
        int                reuse =
            (instance->line->extflags & EX_ML_REPEAT) ||
            (action && action->action == EV_ActionStartLineScript && !(action->type->flags & EV_POSTCLEARSPECIAL));
...
//
static void EV_initSpecHash(EV_SpecHash &hash, ev_binding_t *bindings, size_t numBindings)
{
...
//
static void EV_initSpecNameHash(EV_SpecNameHash &hash, ev_binding_t *bindings, size_t numBindings)
{
    hash.initialize((unsigned int)numBindings);
...
ev_action_t *EV_ActionForSpecial(int special)
{
    ...
    {
    case LEVEL_FORMAT_UDMF_ETERNITY: return EV_UDMFEternityActionForSpecial(special);
    case LEVEL_FORMAT_HEXEN:         return EV_HexenActionForSpecial(special);
    case LEVEL_FORMAT_PSX:           return EV_PSXActionForSpecial(special);
    default:                         return EV_classicFormatActionForSpecial(special);
    }
}
...
int EV_LockDefIDForSpecial(int special)
{
    ...
    else if(LevelInfo.mapFormat == LEVEL_FORMAT_HEXEN || LevelInfo.mapFormat == LEVEL_FORMAT_UDMF_ETERNITY)
    {
...
static bool EV_checkSpac(ev_action_t *action, ev_instance_t *instance)
{
    ...
    else // activation ability is determined by the linedef's flags
    {
        ...
        if(instance->poly)
        {
            ...
            return instance->spac == SPAC_CROSS && (line->extflags & flags) == flags;
        }
        REQUIRE_ACTOR(thing);
        ...
        if(thing->flags4 & MF4_SPACPUSHWALL) // treat as a wall pusher?
        {
            ...
            if(instance->spac == SPAC_PUSH && (!P_LevelIsVanillaHexen() || (flags & (EX_ML_PLAYER | EX_ML_MISSILE))))
            {
                flags = EX_ML_PUSH;
            }
        }
        ...
        {
        case SPAC_CROSS:  flags = EX_ML_CROSS; break;
        case SPAC_USE:    flags = EX_ML_USE; break;
        case SPAC_IMPACT: flags = EX_ML_IMPACT; break;
        case SPAC_PUSH:   flags = EX_ML_PUSH; break;
        }
...
static bool EV_checkSectorActionSpac(ev_action_t *action, ev_instance_t *instance)
{
    ...
    else // activation ability is determined by the linedef's flags
    {
        ...
        {
        case SEAC_ENTER: flags = SEC_ACTION_ENTER; break;
        case SEAC_EXIT:  flags = SEC_ACTION_EXIT; break;
        }
...
CONSOLE_COMMAND(ev_mapspecials, cf_level)
{
    for(int i = 0; i < numlines; i++)
    {
        ...
        if(action)
        {
            C_Printf("%5d: %3d %3d %s%s\n", i, lines[i].special, lines[i].tag, action->name,
                     action->minversion > 0 ? "*" : "");
        }
    }
}
