...
//------------------------------------------------------------------------------
//
// Purpose: Doom intermission screens.
// Authors: James Haley, Ioan Chera, Max Waine, FozzeY
//
...
inline static bool WI_isFinale()
{
    if(wbs->gotosecret)
    {
        return !LevelInfo.finaleNormalOnly &&
               (LevelInfo.endOfGame ||
                (LevelInfo.finaleSecretType != FINALE_UNSPECIFIED && LevelInfo.finaleSecretType != FINALE_TEXT) ||
                (LevelInfo.finaleSecretType != FINALE_TEXT && LevelInfo.finaleType != FINALE_TEXT));
    }
    return !LevelInfo.finaleSecretOnly && (LevelInfo.endOfGame || LevelInfo.finaleType != FINALE_TEXT);
}
...
static void WI_drawEL()
{
    ...
    {
        patch           = PatchLoader::CacheName(wGlobalDir, wbs->li_nextlevelpic, PU_STATIC);
        loadedInfoPatch = true;
    }
    ...
    {
        // draw "Entering"
        V_DrawPatch((SCREENWIDTH - entering->width) / 2, y, &subscreen43, entering);
...
static void WI_drawAnimatedBack()
{
    ...
    for(i = 0; i < NUMANIMS[epsd]; i++)
    {
        ...
        if(a->ctr >= 0)
            ...
            V_DrawPatch(a->loc.x + (vbscreenyscaled.unscaledw - SCREENWIDTH) / 2, a->loc.y, &vbscreenyscaled,
                        a->p[a->ctr]);
    }
}
...
static void WI_drawShowNextLoc()
{
    ...
    if(estrnonempty(wbs->li_lastexitpic) || (estrnonempty(wbs->li_nextenterpic) && state != StatCount))
    {
        WI_drawEL();
        return;
    }
...
static void WI_prepareEnteringStage()
{
    ...
    else if(GameModeInfo->id == commercial && (estrempty(wbs->li_nextenterpic) || WI_isFinale() || demo_version <= 203))
    {
...
static void WI_drawDeathmatchStats()
{
    ...
    V_DrawPatch(DM_TOTALSX - total->width / 2, DM_MATRIXY - WI_SPACINGY + 10, &subscreen43, total);
    ...
    for(i = 0; i < MAXPLAYERS; i++)
    {
        if(playeringame[i])
        {
            V_DrawPatch(x - p[i]->width / 2, DM_MATRIXY - WI_SPACINGY, &subscreen43, p[i]);

            V_DrawPatch(DM_MATRIXX - p[i]->width / 2, y, &subscreen43, p[i]);

            if(i == me)
            {
                V_DrawPatch(x - p[i]->width / 2, DM_MATRIXY - WI_SPACINGY, &subscreen43, bstar);

                V_DrawPatch(DM_MATRIXX - p[i]->width / 2, y, &subscreen43, star);
            }
        }
        x += DM_SPACINGX;
        y += WI_SPACINGY;
...
static void WI_drawNetgameStats()
{
    ...
    // draw stat titles (top line)
    V_DrawPatch(NG_STATSX + NG_SPACINGX - kills->width, NG_STATSY, &subscreen43, kills);

    V_DrawPatch(NG_STATSX + 2 * NG_SPACINGX - items->width, NG_STATSY, &subscreen43, items);

    V_DrawPatch(NG_STATSX + 3 * NG_SPACINGX - secret->width, NG_STATSY, &subscreen43, secret);

    if(dofrags)
        V_DrawPatch(NG_STATSX + 4 * NG_SPACINGX - frags->width, NG_STATSY, &subscreen43, frags);
...
static void WI_OverlayBackground()
{
    V_ColorBlockTL(&subscreen43, (byte)wi_fade_color, 0, 0, backscreen1.width, backscreen1.height, wi_tl_level);
}
...
static void WI_DrawBackground()
{
    ...
    else if(estrnonempty(wbs->li_lastexitpic) || GameModeInfo->id == commercial ||
            (GameModeInfo->id == retail && !overworld(epsd)))
    {
...
static void WI_initVariables(wbstartstruct_t *wbstartstruct)
{
    ...
    if((!wbs->li_nextlevelpic || !*wbs->li_nextlevelpic) && wbs->li_nextlevelname && *wbs->li_nextlevelname)
    {
    ...
    if(LevelInfo.useEDFInterName || inmanageddir)
    {
        ...
        if(!nextMapName)
        {
            ...
            else
            {
                ...
                {
                    edf_string_t *str;
                    psnprintf(nameBuffer, 24, "_IN_NAME_E%01dM%01d", wbs->nextEpisode + 1, wbs->next + 1);
                    if((str = E_StringForName(nameBuffer)))
                        nextMapName = str->string;
                }
