...
void A_FireMacePL1(actionargs_t *actionargs)
{
   ...
   P_SubtractAmmo(player, -1);
   psp->sx = psp->rsx = ((P_Random(pr_firemace) & 3) - 2) * FRACUNIT;
   psp->sy = psp->rsy = WEAPONTOP + (P_Random(pr_firemace) & 3) * FRACUNIT;
...
void A_GauntletAttack(actionargs_t *actionargs)
{
   ...
   psp->sx = psp->rsx = ((P_Random(pr_gauntlets) & 3) - 2) * FRACUNIT;
   psp->sy = psp->rsy = WEAPONTOP + (P_Random(pr_gauntlets) & 3) * FRACUNIT;
   angle = mo->angle;
