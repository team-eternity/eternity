...
#endif
{
    ...
    Uint32 initflags =
        (M_CheckParm("-nodraw") && (M_CheckParm("-nosound") || (M_CheckParm("-nosfx") && M_CheckParm("-nomusic")))) ?
            SDL_INIT_JOYSTICK :
...
static void VerifySDLVersions()
{
    ...
    if(lv.major != ex_vers[0].major || lv.minor != ex_vers[0].minor || lv.patch < ex_vers[0].patch)
    {
        error |= ERROR_SDL;
        printf("WARNING: SDL linked version is not the expected version\n"
               "%d.%d.%d (linked) != %d.%d.(%d+) (expected)\n",
               lv.major, lv.minor, lv.patch, ex_vers[0].major, ex_vers[0].minor, ex_vers[0].patch);
    }
    ...
    if(lv2->major != ex_vers[1].major || lv2->minor != ex_vers[1].minor || lv2->patch < ex_vers[1].patch)
    {
        error |= ERROR_SDL_MIXER;
        printf("WARNING: SDL_mixer linked version is not the expected version\n"
               "%d.%d.%d (linked) != %d.%d.(%d+) (expected)\n",
               lv2->major, lv2->minor, lv2->patch, ex_vers[1].major, ex_vers[1].minor, ex_vers[1].patch);
    }
    ...
    if(lv2->major != ex_vers[2].major || lv2->minor != ex_vers[2].minor || lv2->patch < ex_vers[2].patch)
    {
        error |= ERROR_SDL_NET;
        printf("WARNING: SDL_net linked version is not the expected version\n"
               "%d.%d.%d (linked) != %d.%d.(%d+) (expected)\n\n",
               lv2->major, lv2->minor, lv2->patch, ex_vers[2].major, ex_vers[2].minor, ex_vers[2].patch);
    }
