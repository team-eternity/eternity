...
int mmus2mid(UBYTE *mus, size_t size, MIDI *mididata, UWORD division, int nocomp)
{
    ...
    for(i = 0; i < MIDI_TRACKS; i++)
    {
        if(mididata->track[i].len)
        {
            ...
            if(!(mididata->track[i].data = erealloc(unsigned char *, mididata->track[i].data, mididata->track[i].len)))
                return MEMALLOC;
        }
        else
        {
...
int MidiToMIDI(UBYTE *mid, MIDI *mididata)
{
    ...
    for(i = 0; i < ntracks; i++)
    {
        ...
        // read a track
        mididata->track[i].data = erealloc(unsigned char *, mididata->track[i].data, mididata->track[i].len);
