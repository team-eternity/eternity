...
void SDLVideoDriver::ReadScreen(byte *scr)
{
    ...
    V_InitVBufferFrom(&temp, vbscreen.width, vbscreen.height, vbscreen.height, video.bitdepth, scr);
...
void SDLVideoDriver::SetPrimaryBuffer()
{
    ...
    if(window)
    {
        ...
        primary_surface =
            SDL_CreateRGBSurfaceWithFormat(0, video.height, video.width + bump, 0, SDL_PIXELFORMAT_INDEX8);
        if(!primary_surface)
        ...
        rgba_surface = SDL_CreateRGBSurfaceWithFormat(0, video.height, video.width + bump, 0, pixelformat);
        if(!rgba_surface)
        {
            I_Error("SDLVideoDriver::SetPrimaryBuffer: failed to create true-colour buffer: %s\n", SDL_GetError());
        }
        sdltexture =
            SDL_CreateTexture(renderer, pixelformat, SDL_TEXTUREACCESS_STREAMING, video.height, video.width + bump);
        if(!sdltexture)
        {
...
//
static double I_calcScaleAndDisplacement(SDL_Renderer *renderer, int width, int height, v2double_t *displacement)
{
    double destScale = 1;
    *displacement    = {};
...
bool SDLVideoDriver::InitGraphicsMode()
{
    ...
    if(geom.screentype == screentype_e::FULLSCREEN || geom.screentype == screentype_e::FULLSCREEN_DESKTOP)
    {
                              ...
                              SDL_WINDOWPOS_CENTERED_DISPLAY(v_displaynum), geom.width, geom.height, window_flags)))
    {
        ...
        if(!(window = SDL_CreateWindow(ee_wmCaption, SDL_WINDOWPOS_CENTERED_DISPLAY(v_displaynum),
                                       SDL_WINDOWPOS_CENTERED_DISPLAY(v_displaynum), fallback_w, fallback_h,
                                       fallback_w_flags)))
        {
