...
bool P_SetMobjState(Mobj *mobj, statenum_t state)
{
    ...
    do
    {
        ...
        st = states[state];
        if(vanilla_heretic && mobj->type == E_SafeThingType(MT_POD) && mobj->health <= 0 && st->tics == 0 &&
           st->nextstate == NullStateNum)
        {
            st->tics   = 1050;
...
static bool P_advancedSkyVanish(Mobj *mo, const sector_t *topedgesec, const zrefs_t *slopebumpz)
{
    ...
    if((ceilingsector->intflags & SIF_SKY || R_IsSkyLikePortalSurface(ceilingsector->srf.ceiling)))
    {
        if(ceilingsector->srf.ceiling.slope)
        {
            // Can hit either vertically or horizontally the slope, unlike the horizontal ceiling

            if(P_SlopesEqual(mo->zref.sector.ceiling, ceilingsector, surf_ceil) && mo->z >= mo->zref.ceiling - theight)
            {
                mo->remove();
                return true;
            }
            if(slopebumpz && P_SlopesEqual(mo->zref.sector.ceiling, slopebumpz->sector.ceiling, surf_ceil) &&
    ...
    if(topedgesec && demo_version >= 342 &&
       (topedgesec->intflags & SIF_SKY || R_IsSkyLikePortalSurface(topedgesec->srf.ceiling)))
    {
...
void P_XYMovement(Mobj *mo)
{
    ...
    do
    {
        ...
        if(!P_TryMove(mo, ptryx, ptryy, true))
        {
            ...
            if(!(mo->flags & MF_MISSILE) && demo_version >= 203 &&
               (mo->flags & MF_BOUNCES || (!player && clip.blockline && variable_friction && mo->z <= mo->zref.floor &&
                                           P_GetFriction(mo, nullptr) > ORIG_FRICTION)))
            {
                if(clip.blockline)
                {
                    fixed_t r =
                        ((clip.blockline->dx >> FRACBITS) * mo->momx + (clip.blockline->dy >> FRACBITS) * mo->momy) /
            ...
            else if(demo_version <= 203 && !vanilla_heretic ? !!player :
                                                              !!(mo->flags3 & MF3_SLIDE)) // haleyjd: SLIDE flag
            {
            ...
            else if(mo->flags & MF_MISSILE)
            {
                ...
                if(clip.BlockingMobj && clip.BlockingMobj->flags2 & MF2_REFLECTIVE)
                {
                    angle_t refangle = P_PointToAngle(clip.BlockingMobj->x, clip.BlockingMobj->y, mo->x, mo->y);
                ...
                if(clip.ceilingline && clip.ceilingline->backsector &&
                   (clip.ceilingline->backsector->intflags & SIF_SKY ||
                    (demo_version >= 342 && clip.ceilingline->extflags & EX_ML_UPPERPORTAL &&
                ...
                if(demo_version >= 342 && clip.blockline && !clip.blockline->backsector && R_IsSkyWall(*clip.blockline))
                {
                    mo->remove();
                    return;
                }

                P_ExplodeMissile(mo, clip.ceilingline ? clip.ceilingline->backsector : nullptr, &clip.zref);
    ...
    if(!P_OnGroundOrThing(*mo) && !(mo->flags4 & MF4_FLY) && (!mo->player || LevelInfo.airFriction == 0))
    {
        return;
    }
    ...
    if(((mo->flags & MF_BOUNCES && mo->z > mo->zref.dropoff) || mo->flags & MF_CORPSE || mo->intflags & MIF_FALLING) &&
       (mo->momx > FRACUNIT / 4 || mo->momx < -FRACUNIT / 4 || mo->momy > FRACUNIT / 4 || mo->momy < -FRACUNIT / 4))
    {
        ...
        if(mo->zref.floor != floorsector->srf.floor.getZAt(pos))
        {
            ...
            {
                if(floorsector == mofloorsector ||
                   (floorsector->srf.floor.slope &&
       ...
       (!player || !(player->cmd.forwardmove | player->cmd.sidemove) || (player->mo != mo && demo_version >= 203)))
    {
        ...
        if(player && E_PlayerInWalkingState(player) && (player->mo == mo || demo_version < 203))
...
void P_PlayerHitFloor(Mobj *mo, bool onthing)
{
    ...
    if(demo_version < 329 || mo->health > 0)
    {
        if(!getComp(comp_fallingdmg) && demo_version >= 329)
        {
            ...
            {
                if(!mo->player->powers[pw_invulnerability].isActive() && !(mo->player->cheats & CF_GODMODE))
...
static bool P_planeBounce(Mobj &thing, surf_e surf, bool floordecay)
{
    ...
    if(thing.zref.sector[surf])
    {
        ...
        if(thing.flags & MF_NOGRAVITY && slope && slope->zdelta)
        {
            ...
            fixed_t prod = FixedMul(slope->normal.x, thing.momx) + FixedMul(slope->normal.y, thing.momy) +
...
static void P_ZMovement(Mobj *mo)
{
    ...
    if(mo->flags & MF_BOUNCES && (mo->momz || horizontalBounceOnSlope))
    {
        ...
        if(mo->flags & MF_MISSILE)
        {
            ...
            if(clip.ceilingline && clip.ceilingline->backsector && !clip.ceilingline->backsector->srf.ceiling.slope &&
            ...
            else
            {
                ...
                P_ExplodeMissile(mo, clip.ceilingline ? clip.ceilingline->backsector : nullptr);
            }
        }
    ...
    if(mo->player && mo->player->mo == mo && // killough 5/12/98: exclude voodoo dolls
       mo->z < mo->zref.floor)
    {
        mo->player->viewheight      -= mo->zref.floor - mo->z;
        mo->player->deltaviewheight  = (mo->player->pclass->viewheight - mo->player->viewheight) >> 3;
    }
...
floater:
    ...
    if(!((mo->flags ^ MF_FLOAT) & (MF_FLOAT | MF_SKULLFLY | MF_INFLOAT)) && mo->target) // killough 11/98: simplify
    {
        fixed_t delta;
        ...
        if((moving_down = (mo->momz < 0)) && !(mo->flags4 & MF4_HERETICBOUNCES))
        {
            ...
            if(mo->flags & MF_TOUCHY && mo->intflags & MIF_ARMED && mo->health > 0)
                P_DamageMobj(mo, nullptr, nullptr, mo->health, MOD_UNKNOWN);
                    ...
                    mo->player->mo == mo && mo->momz < -LevelInfo.gravity * kHitFloorGravityFactor)
            {
...
void P_NightmareRespawn(Mobj *mobj)
{
    ...
    sourcepos.x                 += mobj->x;
    sourcepos.y                 += mobj->y;
    mo = P_SpawnMobj(mobj->x, mobj->y, floorsector->srf.floor.getZAt(sourcepos) + GameModeInfo->teleFogHeight,
    ...
    mo = P_SpawnMobj(x, y, ss->sector->srf.floor.getZAt(x, y) + GameModeInfo->teleFogHeight,
                     E_SafeThingName(GameModeInfo->teleFogType));
...
static const line_t *P_avoidPortalEdges(Mobj &mobj, surf_e surf)
{
    ...
    for(int i = 0; i < sector.linecount; ++i)
    {
        ...
        divline_t dl = { mobj.prevpos.x, mobj.prevpos.y, mobj.x - mobj.prevpos.x, mobj.y - mobj.prevpos.y };
...
inline static void P_checkMobjProjections(Mobj &mobj)
{
    ...
    bool     xychanged =
        mobj.x != mobj.sprojlast.pos.x || mobj.y != mobj.sprojlast.pos.y || mobj.xscale != mobj.sprojlast.xscale;
    if(useportalgroups && (mobj.z != mobj.sprojlast.pos.z || xychanged || spritecomp != mobj.sprojlast.sprite ||
...
static void P_updatePortalSplashAfterTransfer(portalSplash_t &splash, const Mobj &mobj)
{
    ...
    if(splash.passSurface == surf_ceil && P_IsLiquidOverlaylinkedPortal(mobj.subsector->sector->srf.floor))
    {
    ...
    if(splash.passSurface == surf_floor && P_IsLiquidOverlaylinkedPortal(mobj.subsector->sector->srf.floor) &&
       !splash.oldWaterState && !splash.waterState)
    {
        splash.waterState  = mobj.z < P_PortalZ(mobj.subsector->sector->srf.floor, mobj.x, mobj.y);
        splash.sectorAbove = mobj.subsector->sector;
...
void Mobj::Think()
{
    ...
    if(momz || clip.BlockingMobj || lz != zref.floor)
    {
        if(P_Use3DClipping() && ((flags3 & MF3_PASSMOBJ) || (flags & MF_SPECIAL)))
        {
            ...
            else
            {
                ...
                {
                    if(!(leveltime & onmo->info->topdamagemask) && (!player || !player->powers[pw_ironfeet].isActive()))
                    {
                        P_DamageMobj(this, onmo, onmo, onmo->info->topdamage, onmo->info->mod);
                    }
                }

                if(player && this == player->mo && momz < -LevelInfo.gravity * kHitFloorGravityFactor)
                {
    ...
    if(mbf21_demo)
    {
        ...
        if(sector->flags & SECF_MONSTERDEATH && z <= zref.floor && player == nullptr && (flags & MF_SHOOTABLE) &&
           !(flags & MF_FLOAT))
        {
    ...
    if(!vanilla_heretic && info->crashstate != NullStateNum && flags & MF_CORPSE && !(intflags & MIF_CRASHED) &&
       z <= zref.floor)
    {
        intflags |= MIF_CRASHED;
        P_SetMobjState(this, info->crashstate);
    ...
    {
        if(((tics == 0) && (state->flags & STATEFI_DECORATE) && !(state->flags & STATEFI_VANILLA0TIC)) || !--tics)
    ...
    else
    {
        ...
        bool can_respawn = flags & MF_COUNTKILL && (respawnmonsters || (flags2 & (MF2_ALWAYSRESPAWN | MF2_REMOVEDEAD)));
        ...
        if(can_respawn && movecount >= info->respawntime && !(leveltime & 31) &&
           P_Random(pr_respawn) <= info->respawnchance)
        {
...
void Mobj::ThinkRavenFast()
{
    ...
    // Handle movement
    if(this->momx || this->momy || (this->z != this->zref.floor) || this->momz)
    {
        xfrac    = this->momx >> 3;
        yfrac    = this->momy >> 3;
        zfrac    = this->momz >> 3;
...
// P_SpawnMobj
//
Mobj *P_SpawnMobj(fixed_t x, fixed_t y, fixed_t z, mobjtype_t type, bool nolastlook)
{
    Mobj       *mobj = new Mobj;
    ...
    if(z == FLOATRANDZ)
    {
        ...
        {
            space   -= 40 * FRACUNIT;
            mobj->z  = ((space * P_Random(pr_spawnfloat)) >> 8) + mobj->zref.floor + 40 * FRACUNIT;
        }
        else
...
void Mobj::remove()
{
    ...
    if((this->flags & MF_SPECIAL) || ((dmflags & DM_BARRELRESPAWN) && this->type == E_ThingNumForDEHNum(MT_BARREL)))
    {
...
Mobj *P_SpawnMapThing(mapthing_t *mthing)
{
    ...
    if(demo_compatibility || (demo_version >= 203 && mthing->options & MTF_RESERVED))
    ...
    if(mthing->type == 11)
    {
        ...
        if(offset >= num_deathmatchstarts)
        {
            ...
            deathmatchstarts =
                erealloc(mapthing_t *, deathmatchstarts, num_deathmatchstarts * sizeof(*deathmatchstarts));
    ...
    if(mthing->type <= 4 && mthing->type > 0) // killough 2/26/98 -- fix crashes
    {
        ...
        if(GameType == gt_single && mthing->type > 1 && mthing->type <= dogs + 1 &&
    ...
    if(i == -1 || i == NUMMOBJTYPES)
    {
        if(!p_markunknowns)
        {
            doom_printf(FC_ERROR "Unknown thing type %d at (%.2f, %.2f)", mthing->type, M_FixedToDouble(mthing->x),
                        M_FixedToDouble(mthing->y));
        }
        else
        {
            // Still print the error, but only at console
            C_Printf(FC_ERROR "Unknown thing type %d at (%.2f, %.2f)", mthing->type, M_FixedToDouble(mthing->x),
                     M_FixedToDouble(mthing->y));
...
spawnit:
    ...
    if(!(mobj->flags & MF_FRIEND) && mthing->options & MTF_FRIEND && demo_version >= 203)
    {
    ...
    if(mobjinfo[i]->parent && mobjinfo[i]->parent->index == E_GetThingNumForName("EESectorActionProto"))
                  ...
                  const MetaTable *pufftype, const Mobj *hitmobj)
{
    ...
    if(hitmobj)
    {
        ...
        if(estrnonempty(altname))
        {
            ...
            if(otable)
            {
                ...
                const char *althitsound = pufftype->getString(keyPuffHitSound, nullptr);
    ...
    if(th)
    {
        ...
        S_StartSoundName(th, hitmobj && estrnonempty(hitsound) ? hitsound : pufftype->getString(keyPuffSound, nullptr));
    ...
    if(ptcl && drawparticles && bulletpuff_particle && (trace.attackrange != MELEERANGE || !punchhack))
    {
...
//
BloodSpawner::BloodSpawner(Mobj *ptarget, fixed_t px, fixed_t py, fixed_t pz, int pdamage, const divline_t &dv,
                           Mobj *pinflictor)
...
//
BloodSpawner::BloodSpawner(Mobj *crushtarget, int pdamage) : target(crushtarget), inflictor(nullptr), damage(pdamage)
{
    x   = crushtarget->x;
    y   = crushtarget->y;
...
void BloodSpawner::spawn(bloodaction_e action) const
{
    ...
    switch(behavior)
    {
    case BLOODTYPE_DOOM:       P_spawnBloodDoom(type, *this); break;
    case BLOODTYPE_HERETIC:
    case BLOODTYPE_HEXEN:      P_spawnBloodRaven(type, *this, (behavior == BLOODTYPE_HEXEN)); break;
    case BLOODTYPE_HERETICRIP:
    case BLOODTYPE_HEXENRIP:   P_spawnBloodRip(type, *this, (behavior == BLOODTYPE_HEXENRIP)); break;
    case BLOODTYPE_STRIFE:     P_spawnBloodStrife(type, *this); break;
    case BLOODTYPE_CRUSH:      P_spawnCrusherBlood(target, type); break;
...
bool P_CheckMissileSpawn(Mobj *th)
{
    ...
    v2fixed_t pos        = P_LinePortalCrossing(th->x, th->y, th->momx >> depower, th->momy >> depower, &newgroupid);
...
Mobj *P_SpawnMissileEx(const missileinfo_t &missileinfo)
{
    ...
    {
        mo->momz = P_MissileMomz(missileinfo.destx - source->x, missileinfo.desty - source->y,
                                 missileinfo.destz - source->z, mo->info->speed);
    }
...
fixed_t P_PlayerPitchSlope(player_t *player)
{
    ...
    if(demo_version < 333 || (full_demo_version > make_full_version(339, 21) && player->pitch == 0))
        return 0;
    else
...
//
Mobj *P_SpawnPlayerMissile(Mobj *source, mobjtype_t type, unsigned flags, playertargetinfo_t *targetinfo)
{
    Mobj   *th;
    ...
    {
        S_StartSoundAtVolume(th, th->info->seesound, WEAPON_VOLUME_SILENCED, ATTN_NORMAL, CHAN_AUTO);
    }
    else
...
//
Mobj *P_SpawnMissileWithDest(Mobj *source, Mobj *dest, mobjtype_t type, fixed_t srcz, fixed_t destx, fixed_t desty,
                             fixed_t destz)
{
    missileinfo_t missileinfo;
...
void P_FallingDamage(player_t *player)
{
    ...
    if(player->mo->momz > -39 * FRACUNIT && damage > player->mo->health && player->mo->health != 1)
    {
...
{
    return ((demo_version >= 333 && !getComp(comp_theights) && mi->c3dheight) ? mi->c3dheight : mi->height);
}
...
bool P_RestingOnGround(const Mobj &thing, const surface_t &floor)
{
    ...
    return thing.zref.sector.floor && &thing.zref.sector.floor->srf.floor == &floor && thing.z == thing.zref.floor;
}
...
static cell AMX_NATIVE_CALL sm_thinginfosound(AMX *amx, cell *params)
{
    ...
    while((mo = P_FindMobjFromTID(tid, mo, context->invocationData.trigger)))
    {
        ...
        switch(typenum)
        {
        case 0:  sndnum = mo->info->seesound; break;
        case 1:  sndnum = mo->info->activesound; break;
        case 2:  sndnum = mo->info->attacksound; break;
        case 3:  sndnum = mo->info->painsound; break;
        case 4:  sndnum = mo->info->deathsound; break;
        default: break;
        }
...
static cell AMX_NATIVE_CALL sm_thinggetpos(AMX *amx, cell *params)
{
    ...
    if((mo = P_FindMobjFromTID(tid, mo, context->invocationData.trigger)))
    {
        switch(valuetoget)
        {
        case TPOS_X:        return (cell)mo->x;
        case TPOS_Y:        return (cell)mo->y;
        case TPOS_Z:        return (cell)mo->z;
        case TPOS_ANGLE:    return (cell)AngleToFixed(mo->angle);
        case TPOS_MOMX:     return (cell)mo->momx;
        case TPOS_MOMY:     return (cell)mo->momy;
        case TPOS_MOMZ:     return (cell)mo->momz;
        case TPOS_FLOORZ:   return (cell)mo->zref.floor;
        case TPOS_CEILINGZ: return (cell)mo->zref.ceiling;
        default:            return 0;
        }
    }
    ...
    while((mo = P_FindMobjFromTID(tid, mo, context->invocationData.trigger)))
    {
        ...
        if(silent == TELE_NORMAL)
        {
            ...
            S_StartSound(P_SpawnMobj(mo->x + 20 * finecosine[mo->angle >> ANGLETOFINESHIFT],
                                     ...
                                     mo->z + GameModeInfo->teleFogHeight, E_SafeThingName(GameModeInfo->teleFogType)),
                         GameModeInfo->teleSound);
        }
    }
