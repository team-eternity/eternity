...
// WadDirectory::Clear
//
// [CG] 09/18/11: Clears a WadDirectory of all its data.
//
void WadDirectory::Clear(void)
{
   ...
   if(this->lumpinfo)
   {
      for(i = 0; i < this->numlumps_before_coalescing; i++)
      {
         // [CG] Clear the lump's cache.
         if(this->lumpinfo[i]->cache)
         {
            Z_Free(this->lumpinfo[i]->cache);
            this->lumpinfo[i]->cache = NULL;
         }

         // [CG] If this is an in-memory lump, free its in-memory data.
         if(this->lumpinfo[i]->data)
         {
            Z_Free((void *)this->lumpinfo[i]->data);
            this->lumpinfo[i]->data = NULL;
         }

         // [CG] Add the file handle to the list of open file handles if it
         //      isn't in there already.
         if(this->lumpinfo[i]->file)
         {
            if(handle_count == 0)
            {
               handle_count++;
            ...
            else
            {
               for(j = 0; j < handle_count; j++)
                  if(handles[j] == this->lumpinfo[i]->file)
                     break;
   ...
   // [CG] Reset numlumps.
   this->numlumps = this->numlumps_before_coalescing = 0;
                                          ...
                                          int li_namespace)
{
   ...
   // [CG] 09/18/11: Keep track of this WadDirectory's lump count before they
   //      are coalesced.
   this->numlumps_before_coalescing = this->numlumps;
