...
void LowResCoopScoreboard::display()
{
   int i, x, y;
   ...
   // [CG] Draw header
   psnprintf(s, sizeof(s), "%s", current_game_type->getCapitalizedName());
   ...
   for(i = 1; i < MAXPLAYERS; i++)
   {
      ...
      {
         psnprintf(s, sizeof(s), "%d",
            cs_settings->max_lives - client->stats.total_deaths
         );
      }
      else
      ...
      psnprintf(s, sizeof(s), "%3ums", client->stats.transit_lag);
      V_FontWriteTextColored(
         hud_overfont, s, font_color,
   ...
   for(i = 1; i < MAXPLAYERS; i++)
   {
      ...
      psnprintf(s, sizeof(s), "%s%3ums", FC_TRANS, client->stats.transit_lag);
      V_FontWriteTextColored(
         hud_overfont, s, font_color,
         ping_column_end - V_FontStringWidth(hud_overfont, s), y
      );
...
void LowResDeathmatchScoreboard::display()
{
   int playernum, i, x, y, window_width;
   int window_x, window_y, scores_x, window_height;
   int score_column_end, score_stat_position;
   int frags_deaths_column_end, frags_deaths_stat_position;
   ...
   psnprintf(s, sizeof(s), "[TKV]RussianDumbass  -9999  9999 / 9999  9999ms");
   ...
   // [CG] Calculate frags/deaths stat position.
   frags_deaths_column_end = lag_stat_position - size_of_space;
   psnprintf(s, sizeof(s), "%s", "0000 / 0000");
   frags_deaths_stat_position =
      frags_deaths_column_end - V_FontStringWidth(hud_overfont, s);

   // [CG] Calculate score stat position.
   score_column_end = frags_deaths_stat_position - size_of_space;
   psnprintf(s, sizeof(s), "%s", "-0000");
   score_stat_position = score_column_end - V_FontStringWidth(hud_overfont, s);
   ...
   if(testing)
   {
      for(i = num_players; i < 17; i++)
      {
         ...
         // [CG] Draw the player's score.
         psnprintf(s, sizeof(s), "%d", -9999);
         V_FontWriteTextColored(
            hud_overfont, s, CR_GREEN,
            score_column_end - V_FontStringWidth(hud_overfont, s), y
         );
         // [CG] Draw the player's frags / deaths.
         psnprintf(s, sizeof(s), "%u / %u", 9999, 9999);
         V_FontWriteTextColored(
            hud_overfont, s, CR_GREEN,
   ...
   for(i = 0; i < num_players; i++)
   {
      ...
      // [CG] Draw the player's score.
      psnprintf(s, sizeof(s), "%d", client->stats.score);
      V_FontWriteTextColored(
         hud_overfont, s, font_color,
         score_column_end - V_FontStringWidth(hud_overfont, s), y
      );
   ...
   for(i = 0; i < num_players; i++)
   {
      ...
      // [CG] Draw the player's score.
      psnprintf(s, sizeof(s), "%s%d", FC_TRANS, client->stats.score);
      V_FontWriteTextColored(
         hud_overfont, s, font_color,
         score_column_end - V_FontStringWidth(hud_overfont, s), y
      );
...
void LowResTeamDeathmatchScoreboard::display()
{
   ...
   for(i = 0; i < num_players; i++)
   {
      ...
      // [CG] Draw the player's lag.
      psnprintf(s, sizeof(s), "%3u", client->stats.transit_lag);
      V_FontWriteTextColored(
         hud_overfont, s, font_color,
         lag_x - V_FontStringWidth(hud_overfont, s), stat_y
      );
   ...
   for(i = 0; i < num_players; i++)
   {
      ...
      // [CG] Draw the player's lag.
      psnprintf(s, sizeof(s), "%s%3u", FC_TRANS, client->stats.transit_lag);
      V_FontWriteTextColored(
         hud_overfont, s, font_color,
         lag_x - V_FontStringWidth(hud_overfont, s), stat_y
      );
