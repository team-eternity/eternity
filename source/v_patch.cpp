...
static void V_drawMaskedColumn(cb_patch_column_t &patchcol, const int ytop, column_t *column)
{
    for(; column->topdelta != 0xff; column = (column_t *)((byte *)column + column->length + 4))
    {
        ...
        if((column->length > 0 && patchcol.y2 < patchcol.y1) || patchcol.y2 >= patchcol.buffer->height)
...
byte *V_PatchToLinear(patch_t *patch, bool flipped, byte fillcolor, int *width, int *height)
{
    ...
    {
        const column_t *column = (const column_t *)((byte *)patch + patch->columnofs[col]);
...
//
patch_t *V_LinearToPatch(byte *linear, int w, int h, size_t *memsize, int tag, void **user)
{
    int       x, y;
    patch_t  *p;
    column_t *c;
    ...
    size_t total_size = 4 * sizeof(int16_t) + w * (h + sizeof(int32_t) + sizeof(column_t) + 3);
...
bool V_WritePatchAsPNG(const char *lump, const char *filename, byte fillcolor)
{
    ...
    // convert to linear
    linear = V_PatchToLinear(reinterpret_cast<patch_t *>(data), false, fillcolor, &width, &height);
