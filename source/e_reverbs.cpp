...
// range inclusive.
//
static void E_getFloatAndClamp(cfg_t *const cfg, const char *name, double &dest, double min, double max, const bool def)
{
    // Test if the item is defined explicitly, or the default should be gotten
    const auto IS_SET = [cfg, def](const char *const s) -> bool { return def || cfg_size((cfg), (s)) > 0; };
...
static void E_processReverb(cfg_t *const sec)
{
    ...
    // Test if the item is defined explicitly, or the default should be gotten
    const auto IS_SET = [sec, &def](const char *const s) -> bool { return def || cfg_size((sec), (s)) > 0; };
