...
bool P_TestMobjZ(Mobj *mo, doom_mapinter_t &clip, Mobj **testz_mobj)
{
    ...
    if(!P_TransPortalBlockWalker(bbox, mo->groupid, true, &data, [](int x, int y, int groupid, void *data) -> bool {
...
//
static bool P_SBlockThingsIterator(int x, int y, bool (*func)(Mobj *), Mobj *actor, int groupid = R_NOGROUP)
{
    Mobj *mobj;
...
static bool PIT_CheckThing3D(Mobj *thing) // killough 3/26/98: make static
{
    ...
    if(D_abs(thing->x - link->x - clip.x) >= blockdist || D_abs(thing->y - link->y - clip.y) >= blockdist)
    ...
    if(clip.thing->groupid != thing->groupid)
    {
        ...
        v2fixed_t pos = P_LinePortalCrossing(*clip.thing, clip.x - clip.thing->x, clip.y - clip.thing->y, &finalgroup);
        P_LinePortalCrossing(pos, thing->x - link->x - pos.x, thing->y - link->y - pos.y, &finalgroup);

        if(finalgroup != thing->groupid &&
           !P_ThingReachesGroupVertically(thing, finalgroup, clip.thing->z + clip.thing->height / 2))
        {
            return true;
        }
    }
    ...
    {
        if(vanilla_heretic || thing->z < clip.thing->z + clip.thing->height - STEPSIZE)
        {
            return P_CheckPickUp(thing);
        }
    }
...
bool P_CheckPosition3D(Mobj *thing, fixed_t x, fixed_t y, PODCollection<line_t *> *pushhit)
{
    ...
    if(!P_TransPortalBlockWalker(
           bbox, thing->groupid, true, [thing, realheight, &thingblocker](int x, int y, int groupid) -> bool {
               // haleyjd: from zdoom:
               Mobj *robin = nullptr;
               ...
               do
               {
                   if(!P_SBlockThingsIterator(x, y, PIT_CheckThing3D, robin, groupid))
                   {
                       ...
                       else if(!clip.BlockingMobj->player && !(thing->flags & (MF_FLOAT | MF_MISSILE | MF_SKULLFLY)) &&
                       ...
                       else if(thing->player && thing->z + thing->height - clip.BlockingMobj->z <= STEPSIZE)
                       {
                           if(thingblocker)
                           {
    ...
    auto checkLine3DVisit = [](int x, int y, int groupid, void *data) -> bool {
    ...
    if(!P_TransPortalBlockWalker(bbox, thing->groupid, true, &pcl, checkLine3DVisit))
        return false;
...
static bool P_AdjustFloorCeil(Mobj *thing, bool midtex)
{
    ...
    if(noseclerptic == gametic || (demo_version >= 342 && portalled && !camera && thing == players[displayplayer].mo))
    {
...
static bool PIT_FindAboveIntersectors(Mobj *thing, void *context)
{
    ...
    // Be portal aware
    const linkoffset_t *link = P_GetLinkOffset(data.clip.thing->groupid, thing->groupid);

    // Didn't hit thing?
    if(D_abs(thing->x - link->x - data.clip.x) >= blockdist || D_abs(thing->y - link->y - data.clip.y) >= blockdist)
        return true;

    // Thing intersects above the base?
    if(thing->z >= data.clip.thing->z && thing->z <= data.clip.thing->z + data.clip.thing->height)
    {
...
static bool PIT_FindBelowIntersectors(Mobj *thing, void *context)
{
    ...
    if(thing->z + thing->height <= clip.thing->z + clip.thing->height && thing->z + thing->height > clip.thing->z)
    {
...
void P_FindAboveIntersectors(Mobj *actor, doom_mapinter_t &clip, MobjCollection &coll)
{
    ...
    auto findAboveIntersectorsVisit = [](int x, int y, int groupid, void *data) -> bool {
        return P_BlockThingsIterator(x, y, groupid, PIT_FindAboveIntersectors, data);
    };

    if(!P_TransPortalBlockWalker(bbox, actor->groupid, true, &data, findAboveIntersectorsVisit))
        return;
...
static void P_FindBelowIntersectors(Mobj *actor)
{
    ...
    auto findBelowIntersectorsVisit = [](int x, int y, int groupid, void *data) -> bool {
        return P_BlockThingsIterator(x, y, groupid, PIT_FindBelowIntersectors);
    };

    if(!P_TransPortalBlockWalker(bbox, actor->groupid, true, nullptr, findBelowIntersectorsVisit))
        return;
...
static void PIT_FloorDrop(Mobj *thing)
{
    ...
    if(thing->momz == 0 && (!(thing->flags & MF_NOGRAVITY) || thing->z == oldfloorz))
    {
        ...
        else if((thing->flags & MF_NOGRAVITY) || thing->z - thing->zref.floor <= moveamt)
        {
...
static void PIT_FloorRaise(Mobj *thing)
{
    ...
    if(thing->z <= thing->zref.floor || (!(thing->flags & MF_NOGRAVITY) && (thing->flags2 & MF2_FLOATBOB)))
    {
...
static void PIT_CeilingRaise(Mobj *thing)
{
    ...
    // stay where they are.
    if(thing->z < thing->zref.floor && thing->z + thing->height >= thing->zref.ceiling - moveamt)
    {
        thing->z = thing->zref.floor;
    ...
    else if(/*(thing->flags3 & MF3_PASSMOBJ) &&*/ !isgood && thing->z + thing->height < thing->zref.ceiling)
    {
