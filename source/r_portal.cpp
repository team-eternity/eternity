...
//------------------------------------------------------------------------------
//
// Purpose: Creating, managing, and rendering portals.
...
static void R_clearPortalWindow(planecontext_t &context, ZoneHeap &heap, const contextbounds_t &bounds,
...
static pwindow_t *newPortalWindow(planecontext_t &planecontext, portalcontext_t &portalcontext, ZoneHeap &heap,
...
//
inline static void R_applyPortalTransformTo(const portal_t *portal, fixed_t &x, fixed_t &y, bool applyTranslation)
{
...
inline static void R_applyPortalTransformTo(const portal_t *portal, float &x, float &y, bool applyTranslation)
{
    if(portal->type == R_ANCHORED || portal->type == R_TWOWAY)
    {
        const portaltransform_t &tr = portal->data.anchor.transform;
...
static void R_calcRenderBarrier(const portal_t *portal, const windowlinegen_t &linegen, windowlinegen_t &translatedgen)
{
...
// have one child.
//
static void R_createChildWindow(planecontext_t &planecontext, portalcontext_t &portalcontext, ZoneHeap &heap,
                 ...
                 float ybottom)
{
    ...
    if(windowbottom > windowtop &&
       (windowtop < 0 || windowbottom < 0 || windowtop >= view.height || windowbottom >= view.height))
    {
...
//
static void R_CalculateDeltas(int markerlinenum, int anchorlinenum, fixed_t *dx, fixed_t *dy, fixed_t *dz)
{
...
static void R_calculateTransform(int markerlinenum, int anchorlinenum, portaltransform_t *transf, bool allowrotate,
...
static void R_incrementWorldPortalID(portalcontext_t &context)
{
    ...
    {
        // it wrapped!
        C_Printf("You managed to render roughly 65,535 skybox, anchored, or linked "
...
portal_t *R_GetAnchoredPortal(int markerlinenum, int anchorlinenum, bool allowrotate, bool flipped, fixed_t zoffset)
{
    ...
    for(rover = portals; rover; rover = rover->next)
    {
        if(rover->type != R_ANCHORED ||
           memcmp(&adata.transform, &rover->data.anchor.transform, sizeof(adata.transform)))
            continue;
...
portal_t *R_GetTwoWayPortal(int markerlinenum, int anchorlinenum, bool allowrotate, bool flipped, fixed_t zoffset)
{
    ...
    for(rover = portals; rover; rover = rover->next)
    {
        if(rover->type != R_TWOWAY || memcmp(&adata.transform, &rover->data.anchor.transform, sizeof(adata.transform)))
            continue;
...
//
static void R_produceKaleidoscopePlane(rendercontext_t &context, ZoneHeap &heap, const pwindow_t *window,
                                       const sector_t *sector)
{
    ...
    visplane_t *vplane = R_FindPlane(
        ...
        sector->intflags & SIF_SKY && sector->sky & PL_SKYFLAT ? sector->sky : sector->srf.ceiling.pic,
        R_GetSurfaceLightLevel(surf_ceil, sector), sector->srf.ceiling.offset, sector->srf.ceiling.scale,
        // haleyjd 01/05/08: flat angle
        sector->srf.ceiling.angle + sector->srf.ceiling.baseangle, sector->srf.ceiling.slope, 0, 255, nullptr);

    vplane = R_CheckPlane(planecontext, heap, vplane, window->minx, window->maxx);
...
//
static void R_produceHorizonPlanes(rendercontext_t &context, ZoneHeap &heap, const pwindow_t *window,
                                   const sector_t *sector)
{
    planecontext_t        &planecontext = context.planecontext;
    viewpoint_t           &viewpoint    = context.view;
    ...
    visplane_t *topplane = R_FindPlane(
        ...
        sector->intflags & SIF_SKY && sector->sky & PL_SKYFLAT ? sector->sky : sector->srf.ceiling.pic,
        R_GetSurfaceLightLevel(surf_ceil, sector), sector->srf.ceiling.offset, sector->srf.ceiling.scale,
        angles[surf_ceil], nullptr, 0, 255, nullptr);
    visplane_t *bottomplane = R_FindPlane(
        context.cmapcontext, planecontext, heap, viewpoint, cb_viewpoint, bounds, sector->srf.floor.height,
        R_IsSkyFlat(sector->srf.floor.pic) && sector->sky & PL_SKYFLAT ? sector->sky : sector->srf.floor.pic,
        R_GetSurfaceLightLevel(surf_floor, sector), sector->srf.floor.offset, sector->srf.floor.scale,
        angles[surf_floor], nullptr, 0, 255, nullptr);
    topplane    = R_CheckPlane(planecontext, heap, topplane, window->minx, window->maxx);
    bottomplane = R_CheckPlane(planecontext, heap, bottomplane, window->minx, window->maxx);
    ...
    for(int x = window->minx; x <= window->maxx; x++)
    {
        ...
        if(window->top[x] <= view.ycenter - 1.0f && window->bottom[x] >= view.ycenter)
        {
...
static void R_renderPrimitivePortal(rendercontext_t &context, pwindow_t *window)
{
    ...
    if(portal->type == R_PLANE)
        R_produceKaleidoscopePlane(context, heap, window, sector);
    else
...
static void R_showTainted(cmapcontext_t &cmapcontext, planecontext_t &planecontext, ZoneHeap &heap,
                          ...
                          const contextbounds_t &bounds, pwindow_t *window)
{
    ...
    if(window->type == pw_line)
    {
        ...
        visplane_t     *topplane =
            R_FindPlane(cmapcontext, planecontext, heap, viewpoint, cb_viewpoint, bounds, sector->srf.ceiling.height,
        ...
        visplane_t *bottomplane =
            R_FindPlane(cmapcontext, planecontext, heap, viewpoint, cb_viewpoint, bounds, sector->srf.floor.height,
                        sector->srf.floor.pic, sector->lightlevel, sector->srf.floor.offset, sector->srf.floor.scale,
...
static void R_renderWorldPortal(rendercontext_t &context, pwindow_t *window)
{
    ...
    for(int i = 0; i < video.width; i++)
    {
        if(window->bottom[i] > window->top[i] && (window->bottom[i] < -1 || window->bottom[i] > viewwindow.height ||
                                                  window->top[i] < -1 || window->top[i] > viewwindow.height))
        {
    ...
    if(!R_SetupPortalClipsegs(bspcontext, bounds, portalcontext.portalrender, window->minx, window->maxx, window->top,
                              window->bottom))
        return;
                                   ...
                                   const surface_t &surface)
{
    ...
    for(pwindow_t *rover = portalcontext.windowhead; rover; rover = rover->next)
        if(rover->portal == surface.portal && rover->type == pw_surface[surf] && rover->planez == surface.height &&
                                 ...
                                 const contextbounds_t &bounds, cb_seg_t &seg, surf_e surf)
{
    ...
    if(plane)
    {
        plane = R_FindPlane(cmapcontext, planecontext, heap, viewpoint, cb_viewpoint, bounds, plane->height,
                            plane->picnum, plane->lightlevel, plane->offs, plane->scale, plane->angle, plane->pslope,
                            plane->bflags, plane->opacity, window->head->poverlay);
    }
    //   if(portal)
...
void portaltransform_t::updateFromLines(bool allowrotate)
{
    ...
    if(allowrotate)
    {
        arot = atan2(flipped ? targetline->ny : -targetline->ny, flipped ? targetline->nx : -targetline->nx) -
...
portal_t *R_GetLinkedPortal(int markerlinenum, int anchorlinenum, fixed_t planez, int fromid, int toid)
{
    ...
    for(rover = portals; rover; rover = rover->next)
    {
        if(rover->type != R_LINKED || ldata.delta != rover->data.link.delta ||
           ldata.fromid != rover->data.link.fromid || ldata.toid != rover->data.link.toid ||
...
void R_SpawnQuickLinePortal(line_t &line)
{
    ...
    {
        if(otherline == &line || otherline->special != line.special || otherline->args[0] != line.args[0])
        {
            continue;
        }
        break;
...
static int R_findFreePortalId()
{
    ...
    qsort(&busylist[0], busylist.getLength(), sizeof(int),
...
void R_DefinePortal(const line_t &line)
{
    ...
    case portaltype_linked:
        ...
        for(destlinenum = -1; (destlinenum = P_FindLineFromTag(anchorid, destlinenum)) >= 0;)
        {
        ...
        {
            portal       = R_GetTwoWayPortal(destlinenum, thislinenum, true, flipped, zoffset);
            mirrorportal = R_GetTwoWayPortal(thislinenum, destlinenum, true, flipped, -zoffset);
        }
        else
        {
            ...
            portal       = R_GetLinkedPortal(destlinenum, thislinenum, planez, fromid, toid);
            mirrorportal = R_GetLinkedPortal(thislinenum, destlinenum, planez, toid, fromid);
        }
        break;
    default: //
    ...
    for(i = 0; i < 3; ++i)
        for(j = 0; j < 3; ++j)
        {
            ...
            for(; *list != -1; ++list)
            {
                ...
                if(D_abs(pline.v2->x - tv1.x) > FRACUNIT / 16 || D_abs(pline.v2->y - tv1.y) > FRACUNIT / 16 ||
                   D_abs(pline.v1->x - tv2.x) > FRACUNIT / 16 || D_abs(pline.v1->y - tv2.y) > FRACUNIT / 16)
                {
                    continue;
                }
...
bool R_IsSkyLikePortalSurface(const surface_t &surface)
{
    return surface.portal && !(surface.pflags & (PF_DISABLED | PF_NOPASS)) &&
           ...
           (surface.portal->type == R_SKYBOX || surface.portal->type == R_HORIZON || surface.portal->type == R_PLANE);
}
...
bool R_IsSkyWall(const line_t &line)
{
    ...
    return R_IsSkyFlat(sides[line.sidenum[0]].midtexture) ||
           (line.portal && !(line.pflags & (PF_DISABLED | PF_NOPASS)) && !(line.extflags & EX_ML_BLOCKALL) &&
            (line.portal->type == R_SKYBOX || line.portal->type == R_HORIZON || line.portal->type == R_PLANE));
}
