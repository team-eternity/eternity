...
static void Polyobj_addVertex(polyobj_t *po, vertex_t *v)
{
    ...
    if(po->numVertices >= po->numVerticesAlloc)
    {
        ...
        po->vertices =
            erealloctag(vertex_t **, po->vertices, po->numVerticesAlloc * sizeof(vertex_t *), PU_LEVEL, nullptr);
        po->origVerts =
            erealloctag(vertex_t *, po->origVerts, po->numVerticesAlloc * sizeof(vertex_t), PU_LEVEL, nullptr);
        po->tmpVerts =
            erealloctag(vertex_t *, po->tmpVerts, po->numVerticesAlloc * sizeof(vertex_t), PU_LEVEL, nullptr);
    }
...
static void Polyobj_findExplicit(polyobj_t *po)
{
    ...
    for(i = 0; i < numlines; ++i)
    {
        if(lines[i].special == explicitspec && lines[i].args[0] == po->id && lines[i].args[1] > 0)
        {
...
static void Polyobj_spawnPolyObj(int num, Mobj *spawnSpot, int id)
{
    ...
    for(i = 0; i < numlines; ++i)
    {
        ...
        if(line->special == startspec && line->args[0] == po->id)
        {
...
static void Polyobj_collectPortals(polyobj_t *po)
{
    ...
    if(po->numPortals)
    {
        po->portals = emalloctag(decltype(po->portals), po->numPortals * sizeof(*po->portals), PU_LEVEL, nullptr);
...
static void Polyobj_moveObjectsInside(const polyobj_t &po, fixed_t dx, fixed_t dy)
{
    ...
    for(size_t i = 0; i < po.numPortals; ++i)
    {
        ...
        if(portal.type != R_LINKED || groupvisit[portal.data.link.toid])
            continue;
        P_ForEachClusterGroup(portal.data.link.fromid, portal.data.link.toid, groupvisit, PolyobjIT_moveObjectsInside,
                              &moved);
    }
...
{
    return mo.info->doomednum == POLYOBJ_SPAWN_DOOMEDNUM || mo.info->doomednum == POLYOBJ_SPAWNCRUSH_DOOMEDNUM ||
...
//
inline static void Polyobj_componentSpeed(int resVel, int angle, int *xVel, int *yVel)
{
...
void PolySlideDoorThinker::serialize(SaveArchive &arc)
{
    ...
    arc << polyObjNum << delay << delayCount << initSpeed << speed << initDistance << distance << initAngle << angle
        << revAngle << momx << momy << closing;
    // ioanch 20160310: fix the thinker reference
    if(arc.isLoading())
        Polyobj_GetForNum(polyObjNum)->thinker = this;
}
...
void PolySwingDoorThinker::serialize(SaveArchive &arc)
{
    ...
    arc << polyObjNum << delay << delayCount << initSpeed << speed << initDistance << distance << closing
        << hasBeenPositive;
    // ioanch 20160310: fix the thinker reference
    if(arc.isLoading())
        Polyobj_GetForNum(polyObjNum)->thinker = this;
...
int EV_DoPolyObjRotate(const polyrotdata_t *prdata)
{
    ...
    {
        doom_printf(FC_ERROR "EV_DoPolyObjRotate: bad polyobj %d", prdata->polyObjNum);
        return 0;
    }
    ...
    while((po = Polyobj_GetMirror(po)))
    {
        ...
        th->speed = (prdata->speed * diracc * prdata->direction * BYTEANGLEMUL) >> 3;
        diracc    = (diracc > 0) ? -1 : 1;
...
int EV_DoPolyObjMove(const polymovedata_t *pmdata)
{
    ...
    {
        doom_printf(FC_ERROR "EV_DoPolyObjMove: bad polyobj %d", pmdata->polyObjNum);
        return 0;
    }
...
int EV_DoPolyObjMoveToSpot(const polymoveto_t &pmdata)
{
    ...
    Polyobj_componentSpeed(th->speed, P_PointToAngle(0, 0, distance.x, distance.y) >> ANGLETOFINESHIFT, &th->velocity.x,
                           &th->velocity.y);
...
int EV_DoPolyDoor(const polydoordata_t *doordata)
{
    ...
    {
        doom_printf(FC_ERROR "EV_DoPolyDoor: bad polyobj %d", doordata->polyObjNum);
        return 0;
    }
    ...
    case POLY_DOOR_SWING: //
        ...
        break;
    default: //
        doom_printf(FC_ERROR "EV_DoPolyDoor: unknown door type %d", doordata->doorType);
        return 0;
    }
