...
void FloorMoveThinker::Think()
{
    ...
    // move the floor
    res = T_MoveFloorInDirection(sector, speed, floordestheight, crush, direction, emulateStairCrush);
...
void FloorMoveThinker::serialize(SaveArchive &arc)
{
    ...
    arc << floordestheight << speed << resetTime << resetHeight << stepRaiseTime << delayTime << delayTimer;
}
...
void ElevatorThinker::serialize(SaveArchive &arc)
{
    ...
    arc << type << direction << floordestheight << ceilingdestheight << speed;
}
...
void PillarThinker::serialize(SaveArchive &arc)
{
    ...
    arc << ceilingSpeed << floorSpeed << floordest << ceilingdest << direction << crush;
}
...
int EV_DoFloor(const line_t *line, int tag, floor_e floortype)
{
    ...
    while((secnum = P_FindSectorFromTag(tag, secnum)) >= 0)
    {
        ...
        case lowerFloor32Turbo:
            ...
            floor->floordestheight = floor->sector->srf.floor.height + 32 * FRACUNIT;
            break;
        ...
        case lowerFloorToNearest:
            ...
            floor->sector          = sec;
            floor->speed           = FLOORSPEED;
            floor->floordestheight = P_FindNextLowestFloor(sec, floor->sector->srf.floor.height);
            break;
        ...
        case turboLowerA:
            ...
            floor->sector          = sec;
            floor->speed           = FLOORSPEED * 4;
            floor->floordestheight = P_FindHighestFloorSurrounding(sec) + 8 * FRACUNIT;
            break;
        ...
        case raiseFloorTurbo:
            ...
            floor->sector          = sec;
            floor->speed           = FLOORSPEED * 4;
            floor->floordestheight = P_FindNextHighestFloor(sec, sec->srf.floor.height);
            break;
        ...
        case raiseFloor512:
            ...
            floor->sector          = sec;
            floor->speed           = FLOORSPEED;
            floor->floordestheight = floor->sector->srf.floor.height + 512 * FRACUNIT;
            break;
        ...
        case raiseToTexture:
        {
            ...
            {
                floor->floordestheight = floor->sector->srf.floor.height + minsize;
            }
            else
            {
                floor->floordestheight = (floor->sector->srf.floor.height >> FRACBITS) + (minsize >> FRACBITS);
        ...
        case lowerAndChange:
            ...
            sec = P_FindModelFloorSector(floor->floordestheight, eindex(sec - sectors));
            if(sec)
            {
                floor->texture = sec->srf.floor.pic;
                // jff 3/14/98 transfer both old and new special
...
//
int EV_DoElevator(const line_t *line, const Mobj *mo, const polyobj_t *po, int tag, elevator_e elevtype, fixed_t speed,
                  fixed_t amount, bool isParam)
{
    ...
    manualElevator:
        ...
        case elevateDown:
            ...
            elevator->floordestheight   = P_FindNextLowestFloor(sec, sec->srf.floor.height);
            elevator->ceilingdestheight = elevator->floordestheight + sec->srf.ceiling.height - sec->srf.floor.height;
            break;

        // elevator up to next floor
        case elevateUp:
            elevator->direction         = plat_up;
            elevator->floordestheight   = P_FindNextHighestFloor(sec, sec->srf.floor.height);
            elevator->ceilingdestheight = elevator->floordestheight + sec->srf.ceiling.height - sec->srf.floor.height;
            break;
        ...
        case elevateCurrent:
            ...
            elevator->ceilingdestheight = elevator->floordestheight + sec->srf.ceiling.height - sec->srf.floor.height;
            elevator->direction         = elevator->floordestheight > sec->srf.floor.height ? plat_up : plat_down;
            break;
        case elevateByValue:
    ...
    manual_pillar:
        ...
        else if(destfloorheight - sector->srf.floor.height > sector->srf.ceiling.height - destceilheight)
        {
            ...
            pillar->ceilingSpeed = FixedDiv(sector->srf.ceiling.height - destceilheight,
                                            FixedDiv(destfloorheight - sector->srf.floor.height, pillar->floorSpeed));
        }
        else
        {
            pillar->ceilingSpeed = pd->speed;
            pillar->floorSpeed   = FixedDiv(destfloorheight - sector->srf.floor.height,
                                            FixedDiv(sector->srf.ceiling.height - destceilheight, pillar->ceilingSpeed));
        }
...
int EV_PillarOpen(const line_t *line, const pillardata_t *pd)
{
    ...
    manual_pillar:
        ...
        if(pillar->ceilingdest - sector->srf.ceiling.height > sector->srf.floor.height - pillar->floordest)
        {
            pillar->ceilingSpeed = pd->speed;
            pillar->floorSpeed =
                FixedDiv(sector->srf.floor.height - pillar->floordest,
                         FixedDiv(pillar->ceilingdest - sector->srf.ceiling.height, pillar->ceilingSpeed));
        }
        else
        {
            pillar->floorSpeed   = pd->speed;
            pillar->ceilingSpeed = FixedDiv(pillar->ceilingdest - sector->srf.ceiling.height,
                                            FixedDiv(sector->srf.floor.height - pillar->floordest, pillar->floorSpeed));
        }
