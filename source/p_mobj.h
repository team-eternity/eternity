...
public:
    ...
    void backupAngle() { prevpos.angle = angle; }
...
Mobj *P_SpawnMobj(fixed_t x, fixed_t y, fixed_t z, mobjtype_t type, bool nolastlook = false);
...
public:
    ...
    BloodSpawner(Mobj *ptarget, fixed_t px, fixed_t py, fixed_t pz, int pdamage, angle_t pdir,
                 Mobj *pinflictor = nullptr)
        : target(ptarget), inflictor(pinflictor), x(px), y(py), z(pz), dir(pdir), damage(pdamage)
    {}

    BloodSpawner(Mobj *ptarget, fixed_t px, fixed_t py, fixed_t pz, int pdamage, const divline_t &dv,
                 Mobj *pinflictor = nullptr);
...
Mobj *P_SpawnPlayerMissile(Mobj *source, mobjtype_t type, unsigned flags = 0, playertargetinfo_t *targetinfo = nullptr);
Mobj *P_SpawnMissileAngle(Mobj *source, mobjtype_t type, angle_t angle, fixed_t momz, fixed_t z);
Mobj *P_SpawnPlayerMissileAngleHeretic(Mobj *source, mobjtype_t type, angle_t angle, unsigned flags = 0,
...
{
    return mobj.z <= mobj.zref.floor ||
           (mobj.z + mobj.height >= mobj.zref.ceiling && mobj.flags & MF_SPAWNCEILING && mobj.flags & MF_NOGRAVITY);
}
