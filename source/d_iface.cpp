...
class ActionHashPimpl : public ZoneObject
{
public:
   EHashTable<InputAction, ENCStringHashKey, &InputAction::key,
              &InputAction::links> actions;

   ActionHashPimpl() : ZoneObject(), actions(ACTIONNUMCHAINS) {}

   virtual ~ActionHashPimpl() { actions.destroy(); }
};
...
{
   // [CG] Actions may only be registered once.
   if(pImpl->actions.objectForKey(action_name))
   {
      I_Error(
   ...
   pImpl->actions.addObject(new InputAction(action_name, this));

   if(pImpl->actions.getLoadFactor() > MAX_LOAD_FACTOR)
      pImpl->actions.rebuild(pImpl->actions.getNumChains() * 2);
}
...
{
   if(pImpl->actions.objectForKey(action_name->constPtr()))
      return true;
...
{
   if(pImpl->actions.objectForKey(action->getName()))
      return true;
...
{
   InputAction *action = pImpl->actions.objectForKey(action_name);
...
{
   return pImpl->actions.objectForKey(action_name);
}

InputAction* InputInterface::getAction(qstring *action_name) const
{
   return pImpl->actions.objectForKey(action_name->constPtr());
}

InputAction* InputInterface::actionIterator(InputAction *action) const
{
   return pImpl->actions.tableIterator(action);
}
   ...
   for(size_t i = 0; i < action_names_length; i++)
   {
      ...
      if(c == ';' || i == (action_names_length - 1))
      {
         ...
         if(token.length())
         {
            ...
            if(!(action = pImpl->actions.objectForKey(action_name)))
            {
               if(print_error)
