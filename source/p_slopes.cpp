...
//
// Purpose: Slopes.
// Authors: Stephen McGranahan, Ioan Chera, Max Waine
//
...
// P_CopySlope
//
// Allocates and places a copy of the given slope structure to the targeted sector surface.
//
void P_CopySlope(const pslope_t &src, sector_t &sector, surf_e surf)
{
...
static void P_initSlopeHeights()
{
    ...
    for(int i = 0; i < numsectors; ++i)
    {
        ...
        {
            pSlopeHeights[i].ceilingdelta = P_GetSlopedSectorCeilingDelta(sector, sector.srf.ceiling.slope);
        }
        ...
        else
        {
            ...
            pSlopeHeights[i].touchheight = sector.srf.ceiling.height - sector.srf.floor.height - mindiff;
        }
    }
}
...
//
static float P_GetExtent(const sector_t &sector, const line_t &line, const v2float_t &o, const v2float_t &d)
{
    float fardist = -1.0f;
...
//
static void P_getSlopeProps(int staticFn, bool &frontfloor, bool &backfloor, bool &frontceil, bool &backceil,
                            const int *args)
{
...
{
    bool frontfloor = false, backfloor = false, frontceil = false, backceil = false;
    ...
    const v2float_t origin = { (line.v2->fx + line.v1->fx) * 0.5f, (line.v2->fy + line.v1->fy) * 0.5f };
    ...
    if(frontfloor || frontceil)
    {
        ...
        if(frontceil)
        {
            ...
            line.frontsector->srf.ceiling.slope = P_MakeSlope(point, direction, dz, &line.frontsector->srf.ceiling,
                                                              surf_ceil, &line.frontsector->groupid);
        }
    }
...
static void P_copyPlane(int tag, sector_t *dest, surf_e type)
{
    for(int secnum = -1; (secnum = P_FindSectorFromTag(tag, secnum)) != -1;)
    {
        ...
        {
            P_CopySlope(*srcsec.srf[type].slope, *dest, type);
            return;
        }
    }
}
...
void P_CopySectorSlope(line_t *line, int staticFn)
{
    ...
    for(i = -1; (i = P_FindSectorFromLineArg0(line, i)) >= 0;)
    {
        ...
        if(copyFloor && !fsec->srf.floor.slope && srcsec->srf.floor.slope)
            P_CopySlope(*srcsec->srf.floor.slope, *fsec, surf_floor);

        if(copyCeiling && !fsec->srf.ceiling.slope && srcsec->srf.ceiling.slope)
            P_CopySlope(*srcsec->srf.ceiling.slope, *fsec, surf_ceil);
    }
...
{
    fixed_t dist = FixedMul(x - slope->o.x, slope->d.x) + FixedMul(y - slope->o.y, slope->d.y);
...
static fixed_t P_DistFromPlane(const v3fixed_t &point, const v3fixed_t &pori, const v3fixed_t &pnormal)
{
