...
int E_IncludePrev(cfg_t *cfg, cfg_opt_t *opt, int argc, const char **argv)
{
    ...
    {
        if(lumpinfo[i]->li_namespace == lumpinfo_t::ns_global && !strncasecmp(lumpinfo[i]->name, cfg->filename, 8))
        {
...
//
int E_SpriteFrameCB(cfg_t *cfg, cfg_opt_t *opt, const char *value, void *result)
{
...
//
int E_IntOrFixedCB(cfg_t *cfg, cfg_opt_t *opt, const char *value, void *result)
{
    char       *endptr;
    const char *dotloc;
    ...
    if(dotloc)
    {
        ...
        if(errno == ERANGE)
        {
            if(cfg)
            {
                cfg_error(cfg, "floating point value for option '%s' is out of range\n", opt->name);
            }
            return -1;
        }
...
//
int E_TranslucCB(cfg_t *cfg, cfg_opt_t *opt, const char *value, void *result)
{
    char       *endptr;
    const char *pctloc;
    ...
    if(pctloc)
    {
        ...
        if(errno == ERANGE || pctvalue < 0 || pctvalue > 100)
        {
            if(cfg)
            {
                cfg_error(cfg, "percentage value for option '%s' is out of range\n", opt->name);
            }
            return -1;
        }
...
//
int E_TranslucCB2(cfg_t *cfg, cfg_opt_t *opt, const char *value, void *result)
{
    char       *endptr;
    const char *pctloc;
...
//
int E_ColorStrCB(cfg_t *cfg, cfg_opt_t *opt, const char *value, void *result)
{
    char *endptr;
    ...
    *(int *)result = (int)strtol(value, &endptr, 0);
    ...
    if(*endptr != '\0')
    {
        ...
        if(sscanf(value, "%d %d %d", &r, &g, &b) != 3)
        {
            if(cfg)
            {
                cfg_error(cfg, "invalid color triplet for option '%s'\n", opt->name);
            }
            return -1;
        }
...
const char *E_ExtractPrefix(const char *value, char *prefixbuf, int buflen)
{
    ...
    if(colonloc)
    {
        ...
        if(!(*strval))
        {
            E_EDFLoggedErr(0, "E_ExtractPrefix: invalid prefix:value %s\n", value);
        }
    }
...
//
static void E_metaDoubleFromCfgFloat(MetaTable *meta, cfg_t *cfg, const char *prop, int n, bool list)
{
    ...
    for(int i = 0; i < numprop; i++)
    {
        ...
        for(auto opt = currcfg->opts; opt->type != CFGT_NONE; opt++)
        {
            ...
            for(int j = 0; j < n; j++)
            {
                switch(opt->type)
                {
                case CFGT_INT:    E_MetaIntFromCfgInt(table, currcfg, opt->name, j, list); break;
                case CFGT_STR:    E_MetaStringFromCfgString(table, currcfg, opt->name, j, list); break;
                case CFGT_BOOL:   E_MetaIntFromCfgBool(table, currcfg, opt->name, j, list); break;
                case CFGT_FLAG:   E_MetaIntFromCfgFlag(table, currcfg, opt->name, j, list); break;
                case CFGT_MVPROP: E_MetaTableFromCfgMvprop(table, currcfg, opt->name, opt->flags & CFGF_MULTI); break;
                default:          break;
                }
            }
        }
...
void E_MetaTableFromCfg(cfg_t *cfg, MetaTable *table, MetaTable *prototype)
{
    ...
    for(auto opt = cfg->opts; opt->type != CFGT_NONE; opt++)
    {
        ...
        for(int i = n; i-- > 0;)
        {
            switch(opt->type)
            {
            case CFGT_INT:     E_MetaIntFromCfgInt(table, cfg, opt->name, i, list); break;
            case CFGT_FLOAT:   E_metaDoubleFromCfgFloat(table, cfg, opt->name, i, list); break;
            case CFGT_STR:     E_MetaStringFromCfgString(table, cfg, opt->name, i, list); break;
            case CFGT_BOOL:    E_MetaIntFromCfgBool(table, cfg, opt->name, i, list); break;
            case CFGT_FLAG:    E_MetaIntFromCfgFlag(table, cfg, opt->name, i, list); break;
            case CFGT_STRFUNC: E_MetaStringFromCfgString(table, cfg, opt->name, i, list); break;
            case CFGT_MVPROP:  E_MetaTableFromCfgMvprop(table, cfg, opt->name, opt->flags & CFGF_MULTI); break;
            default:           break;
            }
        }
    }
...
static void DoPStateColon(tr_pstate_t *pstate)
{
    ...
    else if((tokentype == TR_TOKEN_COMMA || tokentype == TR_TOKEN_END) && pstate->prevstate == TR_PSTATE_DSTBEGIN)
    {
...
inline static constexpr fixed_t RANGE_CLAMP(const fixed_t c, const fixed_t end)
{
    return c <= end ? c : end;
}
