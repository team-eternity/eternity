...
void C_InitBackdrop()
{
    ...
    if(darken)
    {
        V_ColorBlockTL(&cback, GameModeInfo->blackIndex, 0, 0, video.width, video.height, alpha);
    }
}
...
{
    for(input_point = inputtext.getBuffer(); V_FontStringWidth(c_font, input_point) > SCREENWIDTH - 20; input_point++)
        ;
}
...
void C_Ticker()
{
    ...
    if(gamestate != GS_CONSOLE)
    {
        ...
        {
            Console.current_height += (Console.current_target < Console.current_height) ? -c_speed : c_speed;
        }
        else
        {
...
bool C_Responder(event_t *ev)
{
    ...
    case ka_console_up: // previous command
        history_current = (history_current <= 0) ? 0 : history_current - 1;
    ...
    case ka_console_down: // next command
        history_current = (history_current >= history_last) ? history_last : history_current + 1;

        // the last history is an empty string
        inputtext = ((history_current == history_last) ? "" : (char *)history[history_current]);
...
void C_Drawer(void)
{
    ...
    int currentHeight = eclamp(int(round(Console.prev_height + lerp * (Console.current_height - Console.prev_height))),
                               1, SCREENHEIGHT);
    ...
    real_height = cback.scaled ? cback.y2lookup[currentHeight - 1] + 1 : currentHeight;
    ...
    y = currentHeight - ((Console.showprompt && message_pos == message_last) ? c_font->absh : 0) - 1;
    ...
    while(1)
    {
        ...
        if(--count < 0)
            break; // end of message history?
        if(y <= -c_font->absh)
            break; // past top of screen?
    ...
    if(currentHeight > c_font->absh && Console.showprompt && message_pos == message_last)
    {
        ...
        if(message_pos == message_last)
        {
            ...
            psnprintf(tempstr, sizeof(tempstr), "%s%s_", a_prompt, input_point);
        }

        V_FontWriteText(c_font, tempstr, 1, currentHeight - c_font->absh - 1);
    }
}
...
static void C_AddMessage(const char *s)
{
    ...
    *end   = '\0';
    ...
    for(c = (const unsigned char *)s; *c; c++)
    {
        ...
        {
            if((*c >= TEXT_COLOR_MIN && *c <= TEXT_COLOR_MAX) || (*c >= TEXT_COLOR_NORMAL && *c <= TEXT_COLOR_ERROR))
                linecolor = *c;
...
//
static void C_StripColorChars(const unsigned char *src, unsigned char *dest, int len)
{
