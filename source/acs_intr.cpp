...
//
ACSEnvironment::ACSEnvironment() : dir{ nullptr }, global{ getGlobalScope(0) }, hub{ nullptr }, map{ nullptr }
{
    global->active = true;
...
//
ACSVM::Word ACSEnvironment::callSpecImpl(ACSVM::Thread *thread, ACSVM::Word spec, const ACSVM::Word *argV,
                                         ACSVM::Word argC)
{
...
// ACSThread::start
//
void ACSThread::start(ACSVM::Script *script, ACSVM::MapScope *map, const ACSVM::ThreadInfo *infoPtr,
...
static void ACS_loadModules(PODCollection<ACSVM::Module *> &modules, WadDirectory *dir, int lump)
{
    ...
    for(;;)
    {
        ...
        while(lumpItr != lumpEnd && ectype::isSpace(*lumpItr))
            ++lumpItr;
...
// ACS_ExecuteScriptS
//
bool ACS_ExecuteScriptS(const char *str, uint32_t mapnum, const uint32_t *argv, uint32_t argc, Mobj *mo, line_t *line,
                        int side, polyobj_t *po)
{
...
//
bool ACS_ExecuteScriptSAlways(const char *str, uint32_t mapnum, const uint32_t *argv, uint32_t argc, Mobj *mo,
                              line_t *line, int side, polyobj_t *po)
{
    ACSVM::String *name = ACSenv.getString(str, strlen(str));
    ACSVM::ScopeID scope{ ACSenv.global->id, ACSenv.hub->id, mapnum ? mapnum : gamemap };
...
//
uint32_t ACS_ExecuteScriptSResult(const char *str, const uint32_t *argv, uint32_t argc, Mobj *mo, line_t *line,
                                  int side, polyobj_t *po)
{
    ACSVM::String *name = ACSenv.getString(str, strlen(str));
    ACSThreadInfo  info{ mo, line, side, po };
