...
static void P_addPortalHitLine(line_t *ld, polyobj_t *po)
{
    if(clip.numportalhit >= clip.portalhit_max)
    {
        ...
        clip.portalhit =
            erealloc(doom_mapinter_t::linepoly_t *, clip.portalhit, sizeof(*clip.portalhit) * clip.portalhit_max);
    }
...
//
static void P_blockingLineDifferentLevel(line_t *ld, fixed_t thingz, fixed_t thingmid, fixed_t thingtopz,
                                         const lineheights_t &innerheights, const lineheights_t &outerheights,
                                         PODCollection<line_t *> *pushhit)
{
    ...
    if(towards == surf_floor && outerheights.bottomend < clip.zref.ceiling)
    {
        ...
        clip.zref.slope.ceiling  = nullptr;
        clip.ceilingline         = ld;
        clip.blockline           = ld;
    }
    ...
    if(!gGroupVisit[ld->frontsector->groupid]) // portals are guaranteed here
    {
                                                        ...
                                                        ld->frontsector, thingmid, &floorceiling)))
        {
            if(ld->backsector)
            {
                ...
                i2.y  += FixedMul(FRACUNIT >> 12, finesine[angle >> ANGLETOFINESHIFT]);
                if(!(reachedsec = P_PointReachesGroupVertically(i2.x, i2.y, linemid, linegroupid, clip.thing->groupid,
                                                                ld->backsector, thingmid, &floorceiling)))
                {
                    postpone = true;
                }
            }
