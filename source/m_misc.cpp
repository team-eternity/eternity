...
default_t defaults[] = {
    DEFAULT_INT("colour", &default_colour, nullptr, 0, 0, TRANSLATIONCOLOURS, default_t::wad_no,
    ...
    DEFAULT_INT("sound_card", &snd_card, nullptr, SND_DEFAULT, SND_MIN, SND_MAX, default_t::wad_no, SND_DESCR),

    DEFAULT_INT("music_card", &mus_card, nullptr, MUS_DEFAULT, MUS_MIN, MUS_MAX, default_t::wad_no, MUS_DESCR),
    ...
    DEFAULT_INT("i_joysticknum", &i_joysticknum, nullptr, 0, 0, UL, default_t::wad_no, "Joystick device number"),
    ...
    DEFAULT_INT("s_precache", &s_precache, nullptr, 0, 0, 1, default_t::wad_no, "precache sounds at startup"),
    ...
    // killough 2/8/98
    DEFAULT_INT("max_player_corpse", &default_bodyquesize, nullptr, 32, UL, UL, default_t::wad_no,
                "number of dead bodies in view supported (negative value = no limit)"),

    // killough 10/98
    DEFAULT_INT("flashing_hom", &flashing_hom, nullptr, 1, 0, 1, default_t::wad_no,
                "1 to enable flashing HOM indicator"),

    // killough 3/31/98
    DEFAULT_INT("demo_insurance", &default_demo_insurance, nullptr, 2, 0, 2, default_t::wad_no,
    ...
    // sf
    DEFAULT_INT("show_scores", &show_scores, nullptr, 0, 0, 1, default_t::wad_game, "show scores in deathmatch"),

    DEFAULT_INT("lefthanded", &lefthanded, nullptr, 0, 0, 1, default_t::wad_game, "0 - right handed, 1 - left handed"),
    ...
    DEFAULT_BOOL("weapon_hotkey_holding", &weapon_hotkey_holding, nullptr, false, default_t::wad_game,
                 "1 to keep switching weapon while holding hotkey"),
    ...
    // killough 3/1/98
    DEFAULT_INT("monsters_remember", &default_monsters_remember, &monsters_remember, 0, 0, 1, default_t::wad_game,
                "1 to enable monsters remembering enemies after killing others"),

    // killough 7/19/98
    DEFAULT_INT("monster_infighting", &default_monster_infighting, &monster_infighting, 1, 0, 1, default_t::wad_game,
                "1 to enable monsters fighting against each other when provoked"),

    // killough 9/8/98
    DEFAULT_INT("monster_backing", &default_monster_backing, &monster_backing, 0, 0, 1, default_t::wad_game,
                "1 to enable monsters backing away from targets"),

    // killough 9/9/98:
    DEFAULT_INT("monster_avoid_hazards", &default_monster_avoid_hazards, &monster_avoid_hazards, 0, 0, 1,
                default_t::wad_game, "1 to enable monsters to intelligently avoid hazards"),

    DEFAULT_INT("monkeys", &default_monkeys, &monkeys, 0, 0, 1, default_t::wad_game,
                "1 to enable monsters to move up/down steep stairs"),

    // killough 9/9/98:
    DEFAULT_INT("monster_friction", &default_monster_friction, &monster_friction, 1, 0, 1, default_t::wad_game,
    ...
    // killough 3/6/98
    DEFAULT_INT("leds_always_off", &leds_always_off, nullptr, 0, 0, 1, default_t::wad_no,
                "1 to keep keyboard LEDs turned off"),

    // jff 4/3/98 allow unlimited sensitivity
    DEFAULT_FLOAT("mouse_sensitivity_horiz", &mouseSensitivity_horiz, nullptr, 5.0, 0, UL, default_t::wad_no,
                  "adjust horizontal (x) mouse sensitivity"),

    // jff 4/3/98 allow unlimited sensitivity
    DEFAULT_FLOAT("mouse_sensitivity_vert", &mouseSensitivity_vert, nullptr, 5.0, 0, UL, default_t::wad_no,
    ...
    DEFAULT_INT("mouse_accel_threshold", &mouseAccel_threshold, nullptr, 10, 0, UL, default_t::wad_no,
                "threshold at which to apply mouse acceleration (custom acceleration mode only)"),

    DEFAULT_FLOAT("mouse_accel_value", &mouseAccel_value, nullptr, 2.0, 0, UL, default_t::wad_no,
    ...
    DEFAULT_INT("show_messages", &showMessages, nullptr, 1, 0, 1, default_t::wad_no, "1 to enable message display"),

    DEFAULT_INT("message_alignment", &mess_align, nullptr, 0, 0, 2, default_t::wad_no,
                "0 for default, 1 for left-aligned, 2 for centered"),

    DEFAULT_INT("mess_colour", &mess_colour, nullptr, CR_RED, 0, CR_BUILTIN, default_t::wad_no, "messages colour"),
    ...
    DEFAULT_INT("screensize", &screenSize, nullptr, 7, 0, 8, default_t::wad_no, "initial play screen size"),

    // jff 3/6/98 fix erroneous upper limit in range
    DEFAULT_INT("usegamma", &usegamma, nullptr, 0, 0, 4, default_t::wad_no, "screen brightness (gamma correction)"),

    // killough 10/98: preloaded files
    DEFAULT_STR("wadfile_1", &wad_files[0], nullptr, "", default_t::wad_no, "WAD file preloaded at program startup"),

    DEFAULT_STR("wadfile_2", &wad_files[1], nullptr, "", default_t::wad_no, "WAD file preloaded at program startup"),

    DEFAULT_STR("dehfile_1", &deh_files[0], nullptr, "", default_t::wad_no,
                "DEH/BEX file preloaded at program startup"),

    DEFAULT_STR("dehfile_2", &deh_files[1], nullptr, "", default_t::wad_no,
                "DEH/BEX file preloaded at program startup"),

    // haleyjd: auto-loaded console scripts
    DEFAULT_STR("cscript_1", &csc_files[0], nullptr, "", default_t::wad_no,
    ...
    DEFAULT_INT("comp_zombie", &default_comp[comp_zombie], &comp[comp_zombie], 1, 0, 1, default_t::wad_game,
                "Zombie players can exit levels"),

    DEFAULT_INT("comp_infcheat", &default_comp[comp_infcheat], &comp[comp_infcheat], 1, 0, 1, default_t::wad_game,
                "Powerup cheats are not infinite duration"),

    DEFAULT_INT("comp_stairs", &default_comp[comp_stairs], &comp[comp_stairs], 1, 0, 1, default_t::wad_game,
                "Build stairs exactly the same way that Doom does"),

    DEFAULT_INT("comp_telefrag", &default_comp[comp_telefrag], &comp[comp_telefrag], 0, 0, 1, default_t::wad_game,
                "Monsters can telefrag on MAP30"),

    DEFAULT_INT("comp_dropoff", &default_comp[comp_dropoff], &comp[comp_dropoff], 0, 0, 1, default_t::wad_game,
                "Some objects never move over tall ledges"),

    DEFAULT_INT("comp_falloff", &default_comp[comp_falloff], &comp[comp_falloff], 0, 0, 1, default_t::wad_game,
                "Objects don't fall off ledges under their own weight"),

    DEFAULT_INT("comp_staylift", &default_comp[comp_staylift], &comp[comp_staylift], 0, 0, 1, default_t::wad_game,
                "Monsters randomly walk off of moving lifts"),

    DEFAULT_INT("comp_doorstuck", &default_comp[comp_doorstuck], &comp[comp_doorstuck], 0, 0, 1, default_t::wad_game,
                "Monsters get stuck on doortracks"),

    DEFAULT_INT("comp_pursuit", &default_comp[comp_pursuit], &comp[comp_pursuit], 1, 0, 1, default_t::wad_game,
                "Monsters don't give up pursuit of targets"),

    DEFAULT_INT("comp_vile", &default_comp[comp_vile], &comp[comp_vile], 1, 0, 1, default_t::wad_game,
                "Arch-Vile resurrects invincible ghosts"),

    DEFAULT_INT("comp_pain", &default_comp[comp_pain], &comp[comp_pain], 0, 0, 1, default_t::wad_game,
                "Pain Elemental limited to 20 lost souls"),

    DEFAULT_INT("comp_skull", &default_comp[comp_skull], &comp[comp_skull], 1, 0, 1, default_t::wad_game,
                "Lost souls get stuck behind walls"),

    DEFAULT_INT("comp_blazing", &default_comp[comp_blazing], &comp[comp_blazing], 0, 0, 1, default_t::wad_game,
                "Blazing doors make double closing sounds"),

    DEFAULT_INT("comp_doorlight", &default_comp[comp_doorlight], &comp[comp_doorlight], 0, 0, 1, default_t::wad_game,
                "Tagged doors don't trigger special lighting"),

    DEFAULT_INT("comp_god", &default_comp[comp_god], &comp[comp_god], 1, 0, 1, default_t::wad_game,
                "God mode isn't absolute"),

    DEFAULT_INT("comp_skymap", &default_comp[comp_skymap], &comp[comp_skymap], 1, 0, 1, default_t::wad_game,
                "Sky is unaffected by invulnerability"),

    DEFAULT_INT("comp_floors", &default_comp[comp_floors], &comp[comp_floors], 0, 0, 1, default_t::wad_game,
                "Use exactly Doom's floor motion behavior"),

    DEFAULT_INT("comp_model", &default_comp[comp_model], &comp[comp_model], 0, 0, 1, default_t::wad_game,
                "Use exactly Doom's linedef trigger model"),

    DEFAULT_INT("comp_zerotags", &default_comp[comp_zerotags], &comp[comp_zerotags], 0, 0, 1, default_t::wad_game,
                "Linedef effects work with sector tag = 0"),

    // haleyjd
    DEFAULT_INT("comp_terrain", &default_comp[comp_terrain], &comp[comp_terrain], 1, 0, 1, default_t::wad_game,
                "Terrain effects not activated on floor contact"),

    // haleyjd
    DEFAULT_INT("comp_respawnfix", &default_comp[comp_respawnfix], &comp[comp_respawnfix], 0, 0, 1, default_t::wad_game,
                "Creatures with no spawnpoint respawn at (0,0)"),

    // haleyjd
    DEFAULT_INT("comp_fallingdmg", &default_comp[comp_fallingdmg], &comp[comp_fallingdmg], 1, 0, 1, default_t::wad_game,
                "Players do not take falling damage"),

    // haleyjd
    DEFAULT_INT("comp_soul", &default_comp[comp_soul], &comp[comp_soul], 0, 0, 1, default_t::wad_game,
                "Lost souls do not bounce on floors"),

    // haleyjd 02/15/02: z checks (includes,supercedes comp_scratch)
    DEFAULT_INT("comp_overunder", &default_comp[comp_overunder], &comp[comp_overunder], 0, 0, 1, default_t::wad_game,
                "Things not fully clipped with respect to z coord"),

    DEFAULT_INT("comp_theights", &default_comp[comp_theights], &comp[comp_theights], 0, 0, 1, default_t::wad_game,
    ...
    DEFAULT_INT("comp_planeshoot", &default_comp[comp_planeshoot], &comp[comp_planeshoot], 0, 0, 1, default_t::wad_game,
                "Tracer shots cannot hit the floor or ceiling"),

    DEFAULT_INT("comp_special", &default_comp[comp_special], &comp[comp_special], 0, 0, 1, default_t::wad_game,
                "One-time line specials are cleared on failure"),

    DEFAULT_INT("comp_ninja", &default_comp[comp_ninja], &comp[comp_ninja], 0, 0, 1, default_t::wad_game,
                "Silent spawns at W/SW/S-facing DM spots"),
    ...
    DEFAULT_INT("key_pause", &key_pause, nullptr, KEYD_PAUSE, 0, 255, default_t::wad_no, "key to pause the game"),

    DEFAULT_INT("key_chat", &key_chat, nullptr, 't', 0, 255, default_t::wad_no, "key to enter a chat message"),

    DEFAULT_INT("key_chatplayer1", &destination_keys[0], nullptr, 'g', 0, 255, default_t::wad_no,
                "key to chat with player 1"),

    // killough 11/98: fix 'i'/'b' reversal
    DEFAULT_INT("key_chatplayer2", &destination_keys[1], nullptr, 'i', 0, 255, default_t::wad_no,
                "key to chat with player 2"),

    // killough 11/98: fix 'i'/'b' reversal
    DEFAULT_INT("key_chatplayer3", &destination_keys[2], nullptr, 'b', 0, 255, default_t::wad_no,
                "key to chat with player 3"),

    DEFAULT_INT("key_chatplayer4", &destination_keys[3], nullptr, 'r', 0, 255, default_t::wad_no,
                "key to chat with player 4"),

    DEFAULT_INT("automlook", &automlook, nullptr, 0, 0, 1, default_t::wad_no, "set to 1 to always mouselook"),

    DEFAULT_INT("invert_mouse", &invert_mouse, nullptr, 0, 0, 1, default_t::wad_no,
                "set to 1 to invert mouse during mouselooking"),

    DEFAULT_INT("invert_padlook", &invert_padlook, nullptr, 0, 0, 1, default_t::wad_no,
                "set to 1 to invert gamepad looking"),

    DEFAULT_INT("use_mouse", &usemouse, nullptr, 1, 0, 1, default_t::wad_no, "1 to enable use of mouse"),
    ...
    DEFAULT_INT("mouseb_dblc1", &mouseb_dblc1, nullptr, 1, -1, 2, default_t::wad_no,
    ...
    // none
    DEFAULT_INT("mapcolor_exit", &mapcolor_exit, nullptr, 0, 0, 255, default_t::wad_game, "color used for exit lines"),
    ...
    // green
    DEFAULT_INT("mapcolor_sprt", &mapcolor_sprt, nullptr, 112, 0, 255, default_t::wad_game, "color used as things"),
    ...
    DEFAULT_INT("mapcolor_frnd", &mapcolor_frnd, nullptr, 252, 0, 255, default_t::wad_game, "color used for friends"),
    ...
    DEFAULT_BOOL("am_overlay", &automap_overlay, nullptr, false, default_t::wad_no, "overlay automap on top of view"),
    ...
    // killough 11/98
    DEFAULT_INT("message_timer", &message_timer, nullptr, 4000, 0, UL, default_t::wad_no,
                "Duration of normal Doom messages (ms)"),

    DEFAULT_INT("hud_overlayid", &hud_overlayid, nullptr, -1, -1, VDR_MAXDRIVERS - 1, default_t::wad_no,
                "Select HUD overlay (-1 = default, 0 = modern, 1 = boom"),

    // sf : fullscreen hud style
    DEFAULT_INT("hud_overlaylayout", &hud_overlaylayout, nullptr, HUD_FLAT, HUD_OFF, HUD_NUMHUDS - 1,
                default_t::wad_game, "fullscreen hud layout"),

    DEFAULT_INT("hud_enabled", &hud_enabled, nullptr, 1, 0, 1, default_t::wad_game, "fullscreen hud enabled"),
    ...
    DEFAULT_INT("c_speed", &c_speed, nullptr, 10, 1, 200, default_t::wad_no, "console speed, pixels/tic"),

    DEFAULT_INT("c_height", &c_height, nullptr, 100, 0, 200, default_t::wad_no, "console height, pixels"),

    DEFAULT_INT("obituaries", &obituaries, nullptr, 0, 0, 1, default_t::wad_game, "obituaries on/off"),

    DEFAULT_INT("obcolour", &obcolour, nullptr, 0, 0, CR_BUILTIN, default_t::wad_no, "obituaries colour"),
    ...
    DEFAULT_INT("bfg_cloud", &drawbfgcloud, nullptr, 0, 0, 1, default_t::wad_game, "draw particle bfg cloud"),

    DEFAULT_INT("pevent_rexpl", &(particleEvents[P_EVENT_ROCKET_EXPLODE].enabled), nullptr, 0, 0, 1,
                default_t::wad_game, "draw particle rocket explosions"),

    DEFAULT_INT("pevent_bfgexpl", &(particleEvents[P_EVENT_BFG_EXPLODE].enabled), nullptr, 0, 0, 1, default_t::wad_game,
                "draw particle bfg explosions"),

    DEFAULT_INT("stretchsky", &stretchsky, nullptr, 0, 0, 1, default_t::wad_no, "(deprecated; do not use)"),
    ...
    DEFAULT_INT("autoaim", &default_autoaim, &autoaim, 1, 0, 1, default_t::wad_game, "1 to enable autoaiming"),
    ...
    DEFAULT_INT("chasecam_speed", &chasecam_speed, nullptr, 33, 1, 100, default_t::wad_no,
    ...
    DEFAULT_STR("mn_background", &mn_background, nullptr, "default", default_t::wad_startup, "menu background"),

    DEFAULT_STR("wad_directory", &wad_directory, nullptr, ".", default_t::wad_no, "user's default wad directory"),

    DEFAULT_INT("r_columnengine", &r_column_engine_num, nullptr, 0, 0, NUMCOLUMNENGINES - 1, default_t::wad_no,
                "0 = normal"),

    DEFAULT_INT("r_spanengine", &r_span_engine_num, nullptr, 0, 0, NUMSPANENGINES - 1, default_t::wad_no,
                "0 = high precision"),
    ...
    DEFAULT_BOOL("donut_emulation", &donut_emulation, nullptr, false, default_t::wad_no,
    ...
    DEFAULT_INT("wipetype", &wipetype, nullptr, 1, 0, 2, default_t::wad_game, "0 = none, 1 = melt, 2 = fade"),
    ...
    DEFAULT_INT("snd_oplemulator", &adlmidi_emulator, nullptr, ADLMIDI_EMU_DOSBOX, 0, ADLMIDI_EMU_end - 1,
                default_t::wad_no, "OPL3 emulator used for ADLMIDI"),
...
#ifdef _WIN32
    DEFAULT_INT("winmm_reset_type", &winmm_reset_type, nullptr, RESET_TYPE_DEFAULT, RESET_TYPE_DEFAULT, RESET_TYPE_XG,
                default_t::wad_no,
...
// Returns the default_t object, or nullptr if not found.
//
static default_t *M_LookupDefault(defaultfile_t *df, const char *name)
{
    ...
    for(dp = df->defaults[default_hash(df, name)].first; dp && strcasecmp(name, dp->name); dp = dp->next)
        ;
...
static void M_defaultFileWriteError(defaultfile_t *df, const char *tmpfile)
{
    ...
    printf("Warning: could not write defaults to %s: %s\n%s left unchanged\n", tmpfile,
           errno ? strerror(errno) : "(Unknown Error)", df->fileName);
}
...
{
    qstring      tmpfile; // char *tmpfile = nullptr;
    default_t   *dp;
    unsigned int blanks;
    FILE        *f;
    ...
    if(config_help && fprintf(f, ";eternity.cfg format:\n"
...
void M_LoadOptions(const default_t::wad_e minimum_allowed)
{
    ...
    if((lump = W_CheckNumForName("OPTIONS")) != -1)
    {
        ...
        char *buf = nullptr, *p, *options = p = (char *)(wGlobalDir.cacheLumpNum(lump, PU_STATIC));
        while(size > 0)
        {
            int len = 0;
