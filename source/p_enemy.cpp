   ...
   {      // open any specials
      ...
      for(good = false; cc->spechit.getLength();)
      {
         if(P_UseSpecialLine(actor, cc->spechit[cc->spechit.getLength() - 1], 0))
            good |= (cc->spechit.pop() == cc->blockline ? 1 : 2);
         else
            cc->spechit.pop();
      }
...
void P_NewChaseDir(Mobj *actor)
{
   ...
   if(demo_version >= 203)
   {
      ...
      {
         P_DoNewChaseDir(actor, cc->dropoff_deltax, cc->dropoff_deltay);
   ...
   if(cap->cnext != cap)        // Empty list? bail out early
   {
      ...
      {   // Random number of monsters, to prevent patterns from forming
         ...
         for(th = cap->cnext; th != cap; th = th->cnext)
         {
            ...
            {
               if(!PIT_FindTarget(static_cast<Mobj *>(th), NULL)) // If target sighted
                  return true;
            }
         }
      }
...
CONSOLE_COMMAND(vilehit, cf_notnet|cf_level)
{
   ...
   clip->radiusAttack(tc->linetarget, plyr->mo, 70, 70, MOD_UNKNOWN, 0);
