...
static void P_recursiveSound(sector_t *sec, const int soundblocks, Mobj *soundtarget)
{
    ...
    for(int i = 0; i < sec->linecount; i++)
    {
        ...
        if(check->pflags & PS_PASSSOUND && check->frontsector == sec)
        {
            ...
            sector_t *iother = R_PointInSubsector(mid - nudge + v2fixed_t(check->portal->data.link.delta))->sector;
...
bool P_CheckRange(Mobj *actor, fixed_t range)
{
    ...
    return // killough 7/18/98: friendly monsters don't attack other friends
        pl && !(actor->flags & pl->flags & MF_FRIEND) && (P_AproxDistance(tx - actor->x, ty - actor->y) < range) &&
...
bool P_HitFriend(Mobj *actor)
{
    ...
    if(actor->target)
    {
        ...
        if(clip.linetarget && clip.linetarget != actor->target &&
           !((clip.linetarget->flags ^ actor->flags) & MF_FRIEND) && !(clip.linetarget->flags3 & MF3_NOFRIENDDMG))
        {
            hitfriend = true;
        }
    }
...
bool P_CheckMissileRange(Mobj *actor)
{
        ...
        return !(actor->flags & MF_FRIEND) ||
               (actor->target->health > 0 &&
                (!(actor->target->flags & MF_FRIEND) ||
                 (actor->target->player ? monster_infighting || P_Random(pr_defect) > 128 :
    ...
    dist = P_AproxDistance(actor->x - getTargetX(actor), actor->y - getTargetY(actor)) - 64 * FRACUNIT;
...
static bool P_IsOnLift(const Mobj *actor)
{
    ...
    if(sec->tag)
    {
        for(int l = -1; (l = P_FindLineFromTag(sec->tag, l)) >= 0;)
        {
            ...
            else if(EV_CompositeActionFlags(EV_ActionForSpecial(lines[l].special)) & EV_ISMBFLIFT)
            {
                return true;
            }
        }
...
bool P_SmartMove(Mobj *actor)
{
    ...
    if((actor->flags2 & MF2_JUMPDOWN || (actor->type == HelperThing)) && target && dog_jumping &&
    ...
    if((on_lift && P_Random(pr_stayonlift) < 230 && // Stay on lift
        !P_IsOnLift(actor)) ||
       (monster_avoid_hazards && !under_damage && // Get away from damage
        (under_damage = P_IsUnderDamage(actor)) && (under_damage < 0 || P_Random(pr_avoidcrush) < 200)))
    {
...
static void P_DoNewChaseDir(Mobj *actor, fixed_t deltax, fixed_t deltay)
{
    ...
    if(xdir != DI_NODIR && ydir != DI_NODIR &&
       turnaround != (actor->movedir = deltay < 0 ? deltax > 0 ? DI_SOUTHEAST : DI_SOUTHWEST :
                                       deltax > 0 ? DI_NORTHEAST :
                                                    DI_NORTHWEST) &&
       P_TryWalk(actor))
    {
        return;
...
static bool PIT_AvoidDropoff(line_t *line, polyobj_t *po, void *context)
{
    if(line->backsector && // Ignore one-sided linedefs
       ...
       clip.bbox[BOXBOTTOM] < line->bbox[BOXTOP] && P_BoxOnLineSide(clip.bbox, line) == -1)
    {
        v2fixed_t       i1, i2;
...
void P_NewChaseDir(Mobj *actor)
{
    ...
    if(demo_version >= 203)
    {
        if(actor->zref.floor - actor->zref.dropoff > STEPSIZE && actor->z <= actor->zref.floor &&
           !(actor->flags & (MF_DROPOFF | MF_FLOAT)) &&
           (!P_Use3DClipping() || !(actor->intflags & MIF_ONMOBJ)) && // haleyjd: OVER_UNDER
        ...
        else
        {
            ...
            if(actor->flags & target->flags & MF_FRIEND && distfriend << FRACBITS > dist && !P_IsOnLift(target) &&
            ...
            else
            {
                ...
                {
                    // Live enemy target
                    if(monster_backing && actor->info->missilestate != NullStateNum &&
...
static bool PIT_FindTarget(Mobj *mo, void *context)
{
    ...
    if(targ && targ->target == mo && P_Random(pr_skiptarget) > 100 && (targ->flags ^ mo->flags) & MF_FRIEND &&
...
bool P_LookForPlayers(Mobj *actor, int allaround)
{
    ...
    if(GameModeInfo->flags & GIF_HASMADMELEE && GameType == gt_single && players[0].health <= 0)
    {
    ...
    stopc = demo_version < 203 && !demo_compatibility && monsters_remember ? MAXPLAYERS : 2; // killough 9/9/98
...
{
    return actor->flags & MF_FRIEND ? P_LookForMonsters(actor, allaround) || P_LookForPlayers(actor, allaround) :
...
bool P_HelpFriend(Mobj *actor)
{
    ...
    for(th = cap->cnext; th != cap; th = th->cnext)
    {
        ...
        else if(mo->flags & MF_JUSTHIT && mo->target && mo->target != actor->target &&
...
void P_SkullFly(Mobj *actor, fixed_t speed, bool useSeeState)
{
    ...
    dist = P_AproxDistance(getTargetX(actor) - actor->x, getTargetY(actor) - actor->y);
    dist = dist / speed;
    if(dist < 1)
        dist = 1;
...
void A_FogSpawn(actionargs_t *actionargs)
{
    ...
    {
    case 0: mo = P_SpawnMobj(actor->x, actor->y, actor->z, E_SafeThingType(MT_FOGPATCHS)); break;
    case 1: mo = P_SpawnMobj(actor->x, actor->y, actor->z, E_SafeThingType(MT_FOGPATCHM)); break;
    case 2: mo = P_SpawnMobj(actor->x, actor->y, actor->z, E_SafeThingType(MT_FOGPATCHL)); break;
    }
    if(mo)
    {
...
CONSOLE_COMMAND(give, cf_notnet | cf_level)
{
    ...
    for(i = 0; i < itemnum; i++)
    {
        Mobj *mo   = P_SpawnMobj(plyr->mo->x, plyr->mo->y, plyr->mo->z, thingnum);
...
CONSOLE_COMMAND(mdk, cf_notnet | cf_level)
{
    ...
    if(clip.linetarget && !(clip.linetarget->flags2 & MF2_INVULNERABLE) // use 10k damage to
