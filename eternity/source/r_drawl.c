...
#ifdef RANGECHECK 
   if(x < 0 || x >= v_width || column.y1 < 0 || column.y2 >= v_height)
...
{ 
   ...
   {      
      //sf : hires
      *dest  = column.colormap[6*256+dest [fuzzoffset[fuzzpos] ? v_width : -v_width]];
      *dest2 = column.colormap[6*256+dest2[fuzzoffset[fuzzpos] ? v_width : -v_width]];
